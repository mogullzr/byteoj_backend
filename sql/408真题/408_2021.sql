...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/b0f06639e00e491e9594ffc682f17897.svg)
其中next是指向直接后继结点的指针，p是尾指针，q是临时指针。现要删除该链表的第一个元素，正确的语句序列是（ ）。', '[''h->next = h->next->next; q = h->next; free(q);'', ''q = h->next; h->next = h->next->next; free(q);'', ''q = h->next; h->next = q->next; if (p != q) p = h; free(q);'', ''q = h->next; h->next = q->next; if (p == q) p = h; free(q);'']', "['D']", 'D
删除头结点后一个结点的基本流程为q = h->next; h->next = q->next; free(q);，即通过指针操作跳过下一个结点后删除该结点。
由于题目中提到了该链表非空，所以可以确定q = h->next一定不为空。
但是如果链表中只有一个结点的话，我们还需要在删除该结点后，将尾指针指向头结点的位置，即if (p == q) p = h;', 9, 'Mogullzr', '2026-01-29 18:55:23', 9, NULL, '2026-01-29 18:55:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1130', '3864');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3865, '2021年408考研真题第2题', '简单', '2021年408真题', '已知初始为空的队列 Q 的一端仅能进行入队操作，另外一端既能进行入队操作又能进行出队操作。若 Q 的入队序列是 1, 2, 3, 4, 5 , 则不能得到的出队序列是（ ）。', '[''5,4,3,1,2'', ''5,3,1,2,4'', ''4,2,1,3,5'', ''4,1,3,2,5'']', "['D']", 'D
每次入队有两个选择，但是出队只有能一个选择，所以 2 在出队时必然和 1 相邻，3 在出队时必然与 2 相邻，以此类推，可以判断 D 选项是不可能的，因为 1 和 2 没有相邻。', 9, 'Mogullzr', '2026-01-29 18:55:23', 9, NULL, '2026-01-29 18:55:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1142', '3865');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3866, '2021年408考研真题第3题', '简单', '2021年408真题', '已知二维数组A按行优先方式存储，每个元素占用 1 个存储单元。若元素A[0][0]的存储地址是 100,A[3][3]的存储地址是 220 , 则元素A[5][5]的存储地址是（ ）。', '[''295'', ''300'', ''301'', ''306'']', "['B']", 'B
A[3][3]和A[0][0]相差 3 行 3 列，A[5][5]和A[3][3]相差 2 行 2 列，其在内存中的偏移比值为 3 / 2。
由此可以计算&A[5][5]：220 + (220 - 100) / 3 * 2 = 300', 9, 'Mogullzr', '2026-01-29 18:55:23', 9, NULL, '2026-01-29 18:55:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1314', '3866');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3867, '2021年408考研真题第4题', '简单', '2021年408真题', '某森林 F 对应的二叉树为 T , 若 T 的先序遍历序列是 a, b, d, c, e, g, f , 中序遍历序列是 b, d, a, e, g, c, f , 则 F 中树的棵数是（ ）。', '[''1'', ''2'', ''3'', ''4'']', "['C']", 'C
这题考察的是两点，一是二叉树构建，即根据 先序 和 中序 构建二叉树。二是森林转二叉树，构建出二叉树后即可反向转化得到对应的森林。
森林中树的个数 即 二叉树根结点的右结点个数之和。', 9, 'Mogullzr', '2026-01-29 18:55:24', 9, NULL, '2026-01-29 18:55:24', 1, 1);
...
根据Huffman 构建过程来构建得到如下二叉树：89
      /      \\
     52      37
   /   \\    /  \\
  22   30  16  21
 /  \\
10  12带权路径长度为：$(10 + 12) × 3 + (30 + 16 + 21) × 2 = 200$', 9, 'Mogullzr', '2026-01-29 18:55:24', 9, NULL, '2026-01-29 18:55:24', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/96a2ad46f1074804800e6d75972a9fd5.svg)
', '[''16'', ''20'', ''23'', ''25'']', "['D']", 'D
根据AVL 旋转方法可知，这题采用 RL 型旋转，旋转后树的结构为：25
   /  \\
  20   30
 /  \\    \\
16  23    40根结点为 25', 9, 'Mogullzr', '2026-01-29 18:55:25', 9, NULL, '2026-01-29 18:55:25', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/523d7f039a9e4a2c964dfbefec802a2a.svg)
', '[''1'', ''2'', ''3'', ''4'']', "['A']", 'A
依次选择入度为 0 的顶点进行输出，得到拓扑序列，发现只有一种可能性。', 9, 'Mogullzr', '2026-01-29 18:55:26', 9, NULL, '2026-01-29 18:55:26', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/0a86355300544a15ba6ce6a958243fcc.svg)
', '[''26, 3, 14, 6'', ''25, 3, 14, 6'', ''21, 3, 14, 6'', ''15, 3, 14, 6'']', "['C']", 'C
在dijkstra 算法中，每次在 dist 数组中选择一个最小值加入顶点集，并从该顶点出发修改 dist 数组。本题中 dist 数组的变化过程如下：dist[26, 3, ∞, 6] → [25, 3, ∞, 6] → [21, 3, 14, 6]', 9, 'Mogullzr', '2026-01-29 18:55:28', 9, NULL, '2026-01-29 18:55:28', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1129', '3871');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3872, '2021年408考研真题第9题', '简单', '2021年408真题', '在一棵高度为 3 的 3 阶 B 树中，根为第 1 层，若第 2 层中有 4 个关键字，则该树的结点个数最多是（ ）。', '[''11'', ''10'', ''9'', ''8'']', "['A']", 'A
保证特性中的结点数量尽量多，然后每个结点中元素数量尽量多。
3 阶 B 树 中个结点最多有两个元素，三个孩子。', 9, 'Mogullzr', '2026-01-29 18:55:28', 9, NULL, '2026-01-29 18:55:28', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1144', '3872');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3873, '2021年408考研真题第10题', '简单', '2021年408真题', '设数组 S[] = {93, 946, 372, 9, 146,151, 301, 485, 236, 327, 43, 892}, 采用最低位优先（LSD）基数排序将 S 排列成升序序列。第 1 趟分配、收集后，元素 372 之前、之后紧邻的元素分别是 (  )', '[''43,892'', ''236,301'', ''301,892'', ''485,301'']', "['C']", 'C
LSD 基数排序第一趟根据最低位对数组进行排序，得到的结果为 {151, 301, 372, 892, 93, 43, 485, 946, 146, 267, 327}。', 9, 'Mogullzr', '2026-01-29 18:55:28', 9, NULL, '2026-01-29 18:55:28', 1, 1);
...
参考构造初始堆的过程：
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/ca9d70b2554d496e96e23f38cad11181.svg)
', 9, 'Mogullzr', '2026-01-29 18:55:28', 9, NULL, '2026-01-29 18:55:28', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1095', '3874');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3875, '2021年408考研真题第12题', '简单', '2021年408真题', '2017 年公布的全球超级计算机 TOP500 排名中，我国“神威·太湖之光”超级计算机蝉联第一，其浮点运算速度为 93.0146PFLOPS，说明该计算机每秒钟完成的浮点操作次数为（ ）。', '[''$9.3 × 10^{13}$次'', ''$9.3 × 10^{15}$次'', ''$9.3$千万亿次'', ''$9.3$亿亿次'']', "['D']", 'D
PFLOPS=$10^{15}$FLOPS，$93$PFOPS =$9.3 × 10^{16}$FLOPS，一个亿 =$10^{8}$，所以$10^{16}$为 亿亿。', 9, 'Mogullzr', '2026-01-29 18:55:29', 9, NULL, '2026-01-29 18:55:29', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1131', '3875');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3876, '2021年408考研真题第13题', '简单', '2021年408真题', '已知带符号整数用补码表示，变量 x，y，z 的机器数分别为 FFFDH，FFDFH，7FFCH，下列结论中，正确的是（ ）。', '[''若 x、y 和 z 为无符号整数，则 z < x < y'', ''若 x、y 和 z 为无符号整数，则 x < y < z'', ''若 x、y 和 z 为带符号整数，则 x < y < z'', ''若 x、y 和 z 为带符号整数，则 y < x < z'']', "['D']", 'D
若 x, y 和 z 均为无符号整数，则 x > y > z，A 和 B 错误。若 x, y 和 z 均为带符号整数，补码的最高位是符号位，0 表示正数，1 表示负数，因此 z 为正数，而 x 和 y 为负数。对于 x 和 y 的比较，数值位取反加一，可知 x = -3H，y = -21H，故 x > y。', 9, 'Mogullzr', '2026-01-29 18:55:29', 9, NULL, '2026-01-29 18:55:29', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1099', '3876');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3877, '2021年408考研真题第14题', '简单', '2021年408真题', '下列数值中，不能用 IEEE754 浮点格式精确表示的（ ）。', '[''1.2'', ''1.25'', ''2.0'', ''2.5'']', "['A']", 'A
选项 B：$1.25 = 1.01B × 2^{0}$;选项 C：$2.0 = 1.0B × 2$;选项 D:$2.5 = 1.01B × 2^{1}$。因此，选项 B、C 和 D 均可以用IEEE 754 浮点数表示精确表示。选项 A 的十进制小数 1.2 转换成二进制的结果是无限循环小数 1.001100110011…，无法用精度有限的 IEEE 754 格式精确表示。', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1176', '3877');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3878, '2021年408考研真题第15题', '简单', '2021年408真题', '某计算机的存储器总线中有 24 位地址线和 32 位数据线，按字编址，字长为 32 位。若 000000H~3FFFFFH 为 RAM 区，则需要 512K×8 位的 RAM 芯片数为（ ）。', '[''8'', ''16'', ''32'', ''64'']', "['C']", 'C
RAM 区的地址空间为 4FFFFFH =$2^{22}$，用 22 位可以表示。字长为 32 bit。参考主存容量的扩展，如果要用$2^{19}$x 8bit 的 RAM 芯片覆盖这些存储空间的话，需要的芯片个数为$(2^{22}/2^{19}) × (32/8) = 8 × 4 = 32$', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1102', '3878');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3879, '2021年408考研真题第16题', '简单', '2021年408真题', '若计算机主存地址为 32 位，按字节编址，Cache 数据区大小为 32KB，主存块大小为 32B，采用直接映射方式和回写（Write Back）策略，则 Cache 行的位数至少是（ ）。', '[''275'', ''274'', ''258'', ''257'']', "['A']", 'A
由于是直接映射，所以物理地址的结构分为以下几个部分：| Tag（17 bits） | Cache 块号（10 bits） | 块内偏移（5 bits） |Cache 行包含以下几个部分：标记（tag 17bits）、有效位（valid 1bit）、修改位（dirty 1bit）、数据区（32B）Cache 行的大小为 17 + 1 + 1 + 32 * 8 = 275', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3880, '2021年408考研真题第17题', '简单', '2021年408真题', '下列存储器中，汇编语言程序员可见的是（ ）。Ⅰ. 指令寄存器Ⅱ. 微指令寄存器Ⅲ. 基址寄存器Ⅳ. 标志状态寄存器', '[''仅Ⅰ、Ⅱ'', ''仅Ⅰ、IV'', ''仅Ⅱ、Ⅳ'', ''仅Ⅲ、Ⅳ'']', "['D']", 'D
参考汇编程序员可见寄存器，指令寄存器（IR）和 微指令寄存器 都是 CPU 控制的，程序员无法通过汇编代码直接操作。基址寄存器、标志状态寄存器（FLAGS）程序员可见。', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1150', '3880');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3881, '2021年408考研真题第18题', '简单', '2021年408真题', '下列关于数据通路的叙述中，错误的是（ ）。', '[''数据通路包含 ALU 等组合逻辑（操作）元件'', ''数据通路包含寄存器等时序逻辑（状态）元件'', ''数据通路不包含用于异常事件检测及响应的电路'', ''数据通路中的数据流动路径由控制信号进行控制'']', "['C']", 'C
指令执行过程中数据所经过的路径，包括路径上的部件，称为数据通路。ALU、通用寄存器、状态寄存器、Cache、MMU、浮点运算逻辑、异常和中断处理逻辑等，都是指令执行过程中数据流经的部件，都属于数据通路的一部分。数据通路中的数据流动路径由控制部件控制，控制部件根据每条指令功能的不同，生成对数据通路的控制信号。', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1227', '3881');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3882, '2021年408考研真题第19题', '简单', '2021年408真题', '下列关于总线的叙述中，错误的是（ ）。', '[''总线是在两个或多个部件之间进行数据交换的传输介质'', ''同步总线由时钟信号定时，时钟频率不一定等于工作频率'', ''异步总线由握手信号定时，一次握手过程完成一位数据交换'', ''突发（Burst）传送总线事务可以在总线上连续传送多个数据'']', "['C']", 'C
总线是在两个或多个设备之间进行通信的传输介质，A 正确。同步总线是指总线通信的双方采用同一个时钟信号，但是一次总线事务不一定在一个时钟周期内完成，即时钟频率不一定等于工作频率，B 正确。异步总线采用握手的方式进行通信，每次握手的过程完成一次通信，但一次通信往往会交换多位而非一位数据，C 错误。突发总线传输事务是指发送方在传输完地址后，连续进行若干次数据的发送，D 正确。', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1179', '3882');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3883, '2021年408考研真题第20题', '简单', '2021年408真题', '下列选项中不属于 I/O 接口的是（ ）。', '[''磁盘驱动器'', ''打印机适配器'', ''网络控制器'', ''可编程中断控制器'']', "['A']", 'A
I/O 接口处于计算机与外设之间，其功能是接收主机发送的 I/O 控制信号，实现主机和外设之间的信息交换。磁盘驱动器并不属于接口，它是磁盘的一部分，它并不处于外设和主机之间。', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1233', '3883');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3884, '2021年408考研真题第21题', '简单', '2021年408真题', '异常事件在当前指令执行过程中进行检测，中断请求则在当前指令执行后进行检测。下列事件中，相应处理程序执行后，必须回到当前指令重新执行的是（ ）。', '[''系统调用'', ''页缺失'', ''DMA 传送结束'', ''打印机缺纸'']', "['B']", 'B
大部分中断都是在一条指令执行完成后（中断周期）才被检测并处理，除了缺页中断和 DMA 请求。DMA 请求只请求总线的使用权，不影响当前指令的执行，不会导致被中断指令的重新执行；而缺页中断发生在取指或间址等指令执行过程之中，并且会阻塞整个指令。当缺页中断发生后，必须回到这条指令重新执行，以便重新访存。选 B。', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1109', '3884');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3885, '2021年408考研真题第22题', '简单', '2021年408真题', '下列是关于多重中断系统中 CPU 响应中断的叙述，其中错误的是（ ）。', '[''仅在用户态（执行用户程序）下，CPU 才能检测和响应中断'', ''CPU 只有在检测到中断请求信号后，才会进入中断响应周期'', ''进入中断响应周期时，CPU 一定处于中断允许（开中断）状态'', ''若 CPU 检测到中断请求信号，则一定存在未被屏蔽的中断源请求信号'']', "['A']", 'A
中断服务程序在内核态下执行，但这并不代表在用户态下就无法响应中断。若只能在用户态下检测和响应中断，显然无法实现多重中断（中断嵌套），A 错误。在多重中断中，CPU 只有在检测到中断请求信号后（中断处理优先级更低的中断请求信号是检测不到的），才会进入中断响应周期。进入中断响应周期时说明此时 CPU 一定处于中断允许状态，否则无法响应该中断。如果所有中断源都被屏蔽（说明该中断处理优先级最高），则 CPU 不会检测到任何中断请求信号。', 9, 'Mogullzr', '2026-01-29 18:55:30', 9, NULL, '2026-01-29 18:55:30', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1283', '3885');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3886, '2021年408考研真题第23题', '简单', '2021年408真题', '下列指令中，只能在内核态执行的是（ ）。', '[''trap 指令'', ''I/O 指令'', ''数据传送指令'', ''设置断点指令'']', "['B']", 'B
在内核态下，CPU 可执行任何指令，在用户态下 CPU 只能执行非特权指令，而特权指令只能在内核态下执行。常见的特权指令有：有关对 IO 设备操作的指令；有关访问程序状态的指令；存取特殊寄存器指令；其他指令。A、C 和 D 都是提供给用户使用的指令，可以在用户态执行，只是可能会使 CPU 从用户态切换到内核态。', 9, 'Mogullzr', '2026-01-29 18:55:31', 9, NULL, '2026-01-29 18:55:31', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1182', '3886');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3887, '2021年408考研真题第24题', '简单', '2021年408真题', '下列操作中，操作系统在创建新进程时，必须完成的是（ ）。I. 申请空白的进程控制块II. 初始化进程控制块III. 设置进程状态为执行态', '[''仅 I'', ''仅 I、II'', ''仅 I、III'', ''仅 II、III'']', "['B']", 'B
操作系统感知进程的唯一方式是通过进程控制块 PCB，所以创建一个新进程时就是为其申请一个空白的进程控制块，并初始化一些必要的进程信息，如初始化进程标志信息、初始化处理机状态信息、设置进程优先级等。I、Ⅱ正确。创建一个进程时，一般会为其分配除 CPU 外的大多数资源，所以一般是将其设置为就绪态，让其等待调度程序的调度。', 9, 'Mogullzr', '2026-01-29 18:55:31', 9, NULL, '2026-01-29 18:55:31', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3888, '2021年408考研真题第25题', '简单', '2021年408真题', '下列内核的数据结构或程序中，分时系统实现时间片轮转调度需要使用的是（ ）。I. 进程控制块II. 时钟中断处理程序III. 进程就绪队列IV. 进程阻塞队列', '[''仅 II、III'', ''仅 I、IV'', ''仅 I、II、III'', ''仅 I、II、IV'']', "['C']", 'C
在分时系统的时间片轮转中，当系统检测到时钟中断时，会引出时钟中断处理程序调度程序从就绪队列中选择一个进程为其分配时间片，并修改该进程的进程控制块中的进程状态等信息，同时将时间片用完的进程放入就绪队列或让其结束运行。I、II、Ⅲ 正确。阻塞队列中的进程只有被唤醒进入就绪队列后，才能参与调度，所以该调度过程不使用阻塞队列。', 9, 'Mogullzr', '2026-01-29 18:55:31', 9, NULL, '2026-01-29 18:55:31', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3889, '2021年408考研真题第26题', '简单', '2021年408真题', '某系统中磁盘的磁道数为 200 (0~199), 磁头当前在 184 号磁道上。用户进程提出的磁盘访问请求对应的磁道号依次为 184, 187, 176, 182, 199。若采用最短寻道时间优先调度算法 (SSTF) 完成磁盘访问，则磁头移动的距离（磁道数）是（ ）。', '[''37'', ''38'', ''41'', ''42'']', "['C']", 'C
最短寻道时间优先算法总是选择调度与当前磁头所在磁道距离最近的磁道。可以得出访问序列 184,182,187,176,199，从而求出移动距离之和是 0+2+5+11+23=41。', 9, 'Mogullzr', '2026-01-29 18:55:31', 9, NULL, '2026-01-29 18:55:31', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1116', '3889');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3890, '2021年408考研真题第27题', '简单', '2021年408真题', '下列事件中，可能引起进程调度程序执行的是（ ）。I. 中断处理结束II. 进程阻塞III. 进程执行结束IV. 进程的时间片用完', '[''仅 I、III'', ''仅 II、IV'', ''仅 III、IV'', ''I、II、III、 IV'']', "['D']", 'D
当进程状态转化时，需要由进程调度程序执行。
。在时间片调度算法中，中断处理结束后，系统检测当前进程的时间片是否用完，如果用完，则将其设为就绪态或让其结束运行，若就绪队列不空，则调度就绪队列的队首进程执行，I 可能。当前进程阻塞时，将其放入阻塞队列，若就绪队列不空，则调度新进程执行，Ⅱ 可能。进程执行结束会导致当前进程释放 CPU，并从就绪队列中选择一个进程获得 CPU，III 可能。进程时间片用完，会导致当前进程让出 CPU，同时选择就绪队列的队首进程获得 CPU，IV 可能。', 9, 'Mogullzr', '2026-01-29 18:55:31', 9, NULL, '2026-01-29 18:55:31', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/adebab5a307743a7b3be18d47137a12d.svg)
若 P 访问虚拟地址为 02A01H 的存储单元，则经地址变换后得到的物理地址是（）。', '[''00A01H'', ''20A01H'', ''60A01H'', ''80A01H'']', "['C']", 'C
页面大小为 4KB，低 12 位是页内偏移。虚拟地址为 02A01H，页号为 02H，02H 页对应的页表项中存在位为 0，进程 P 分配的页框固定为 2，且内存中已有两个页面存在。根据 CLOCK 算法，选择将 3 号页换出，将 2 号页放入 60H 页框，经过地址变换后得到的物理地址是 60A01H。', 9, 'Mogullzr', '2026-01-29 18:55:32', 9, NULL, '2026-01-29 18:55:32', 1, 1);
...
页表基址寄存器（PTBR，Page Table Base Register）中存储的是进程一级页表的物理地址。', 9, 'Mogullzr', '2026-01-29 18:55:32', 9, NULL, '2026-01-29 18:55:32', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1157', '3892');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3893, '2021年408考研真题第30题', '简单', '2021年408真题', '若目录 dir 下有文件 file1，则为删除该文件内核不必完成的工作是（ ）。', '[''删除 file1 的快捷方式'', ''释放 file1 的文件控制块'', ''释放 file1 占用的磁盘空间'', ''删除目录 dir 中与 file1 对应的目录项'']', "['A']", 'A
当你删除某个目录dir下的文件file1时，操作系统内核必须完成以下几项操作：首先，内核会在目录dir中找到file1对应的目录项，并将其删除。目录项是文件名与其 inode（文件控制块）之间的映射。然后，内核检查这个 inode 是否还有其他硬链接引用。如果file1是最后一个链接，那么系统会释放掉该 inode，以及文件占用的磁盘数据块。也就是说，释放文件控制块（inode）和释放磁盘空间，都是删除文件过程中内核必须做的事情。但如果有其他“快捷方式”指向file1，比如软链接（symbolic link），这些快捷方式本身是独立的文件。删除原文件不会影响这些快捷方式，它们会变成“悬挂链接”（指向一个不存在的文件）。因此，内核在删除文件时不需要去删除这些快捷方式。所以，正确答案是：A. 删除 file1 的快捷方式—— 这是内核不必完成的工作。', 9, 'Mogullzr', '2026-01-29 18:55:33', 9, NULL, '2026-01-29 18:55:33', 1, 1);
...
考虑极端情况，当临界资源数为 n 时，每个进程都拥有 1 个临界资源并等待另一个资源，会发生死锁。当临界资源数为 n+1 时，则 n 个进程中至少有一个进程可以获得 2 个临界资源，顺利运行完后释放自己的临界资源，使得其他进程也能顺利运行，不会产生死锁。', 9, 'Mogullzr', '2026-01-29 18:55:33', 9, NULL, '2026-01-29 18:55:33', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1234', '3894');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3895, '2021年408考研真题第32题', '简单', '2021年408真题', '下列选项中，通过系统调用完成的操作是（ ）。', '[''页置换'', ''进程调度'', ''创建新进程'', ''生成随机整数'']', "['C']", 'C
系统调用是由用户进程发起的，请求操作系统的服务。对于 A，当内存中的空闲页框不够时，操作系统会将某些页面调出，并将要访问的页面调入，这个过程完全由操作系统完成不涉及系统调用。对于 B，进程调度完全由操作系统完成，无法通过系统调用完成。对于 C，创建新进程可以通过系统调用来完成，如 Linux 中通过 fork 系统调用来创建子进程。对于 D，生成随机数只需要普通的函数调用，不涉及请求操作系统的服务，如 C 语言中 random() 函数。', 9, 'Mogullzr', '2026-01-29 18:55:33', 9, NULL, '2026-01-29 18:55:33', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1205', '3895');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3896, '2021年408考研真题第33题', '简单', '2021年408真题', '在 TCP/IP 参考模型中，由传输层相邻的下一层实现的主要功能是（ ）。', '[''对话管理'', ''路由选择'', ''端到端报文段传输'', ''结点到结点流量控制'']', "['B']", 'B
TCP/IP 参考模型中传输层相邻的下一层是网际层。TCP/IP 的网际层使用一种尽力而为的服务，它将分组发往任何网络，并为之独立选择合适的路由，但不保证各个分组有序到达，B 正确。TCP/IP 认为可靠性是端到端的问题（传输层的功能），因此它在网际层仅有无连接、不可靠的通信模式，无法完成结点到结点的流量控制（OSI 参考模型的网络层具有该功能）。对话管理和端到端的报文段传输均为传输层的功能。A、C 和 D 错误。', 9, 'Mogullzr', '2026-01-29 18:55:33', 9, NULL, '2026-01-29 18:55:33', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/e4ba8d7cce0e467e9fa39378d191a1af.svg)
', '[''1011 1001'', ''1101 0001'', ''0010 1110'', ''1011 0110'']', "['A']", 'A
差分曼彻斯特编码常用于局域网传输，其规则是：若码元为 1，则前半个码元的电平与上一码元的后半个码元的电平相同；若码元为 0，则情形相反。差分曼彻斯特编码的特点在于，在每个时钟周期的起始处，跳变则说明该比特是 0，不跳变则说明该比特是 1。根据题 34 图，第 1 个码元的信号波形因缺乏上一码元的信号波形，无法判断是 0 还是 1.但根据后面的信号波形，可以求出第 2~8 个码元为 011 1001，因此选 A。', 9, 'Mogullzr', '2026-01-29 18:55:34', 9, NULL, '2026-01-29 18:55:34', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1255', '3897');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3898, '2021年408考研真题第35题', '简单', '2021年408真题', '现将一个 IP 网络划分为 3 个子网，若其中一个子网是 192.168.9.128/26, 则下列网络中，不可能是另外两个子网之一的是（ ）', '[''192.168.9.0/25'', ''192.168.9.0/26'', ''192.168.9.192/26'', ''192.168.9.192/27'']', "['B']", 'B
本题考察的是子网划分。根据题意，将 IP 网络划分为 3 个子网。其中一个是 192.168.9.128/26。可以简写成 x.x.x.10/26（其中 10 是 128 的二进制 1000 0000 的前两位，因为 26-24=2）。A 选项可以简写成 x.x.x.0/25;B 选项可以简写成 x.x.x.00/26;C 选项可以简写成 x.x.x.11/26;D 选项可以简写成 x.x.x.110/27对于 A 和 C，可以组成 x.x.x.0/25、x.x.x.10/26、x.x.x.11/26 这样 3 个互不重叠的子网对于 D，可以组成 x.x.x.10/26、x.x.x.110/27、x.x.x.111/27 这样 3 个互不重叠的子网。但对于 B，要想将一个 IP 网络划分为几个互不重叠的子网，3 个是不够的，至少需要划分为 4 个子网：x.x.x.00/26、x.x.x.01/26、x.x.x.10/26、x.x.x.11/26。', 9, 'Mogullzr', '2026-01-29 18:55:34', 9, NULL, '2026-01-29 18:55:34', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1140', '3898');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3899, '2021年408考研真题第36题', '简单', '2021年408真题', '若路由器向 MTU = 800B 的链路转发一个总长度为 1580B 的 IP 数据报（首部长度为 20B）时，进行了分片，且每个分片尽可能大，则第 2 个分片的总长度字段和 MF 标志位的值分别是（  ）', '[''796,0'', ''796, 1'', ''800,0'', ''800, 1'']', "['B']", 'B
本题考察IP 数据报分片。链路层 MTU = 800B。IP 分组首部长 20B。片偏移以 8 个字节为偏移单位，因此除最后个分片，其他每个分片的数据部分长度都是 8B 的整数倍。所以，最大 IP 分片的数据部分长度为 776B。总长度 1580B 的卫 P 数据报中，数据部分占 1560B，1560B/776B=2.01…需要分成 3 片。故第 2 个分片的总长度字段为 796，MF 为 1（表示还有后续的分片）。', 9, 'Mogullzr', '2026-01-29 18:55:34', 9, NULL, '2026-01-29 18:55:34', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/4e7c3c351e384543949fe6753cb54d9f.svg)
', '[''9, 10, 12, 6'', ''9, 10, 28, 20'', ''9, 20, 12, 20'', ''9, 20, 28, 20'']', "['D']", 'D
根据距离向量算法，E 收到相邻路由器的距离向量后，更新它的路由表：① 当原路由表中没有目的网络时，把该项目添加到路由表中。② 发来的路由信息中有一条到达某个目的网络的路由，该路由与当前使用的路由相比，有较短的距离，就用经过发送路由信息的结点的新路由替换。分析题意可知，E 与邻居路由器 A、B、C 和 D 之间的直接链路距离分别是 8.10.12 和 6.到达 Net1~Net4 没有直接链路，需要通过邻居路由器。从上述算法可知，E 到达目的网络一定是经过 A,B,C 和 D 中距离最小的。根据题中所给的距离信息，计算 E 经邻居路由器到达目的网络 Net1~Net4 的距离，如下表所示，选择到达每个目的网络距离的最短值。| 目的网络 | 经过 A 需要的距离 | 经过 B 需要的距离 | 经过 C 需要的距离 | 经过 D 需要的距离 |
|:----:|:----------:|:----------:|:----------:|:----------:|
| Net1 | 9          | 33         | 32         | 28         |
| Net2 | 20         | 45         | 42         | 34         |
| Net3 | 32         | 28         | 28         | 42         |
| Net4 | 44         | 40         | 20         | 30         |
', 9, 'Mogullzr', '2026-01-29 18:55:36', 9, NULL, '2026-01-29 18:55:36', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1316', '3900');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3901, '2021年408考研真题第38题', '简单', '2021年408真题', '若客户首先向服务器发送 FIN 段请求断开 TCP 连接，则当客户收到服务器发送的 FIN 段并向服务器发送了 ACK 段后，客户的 TCP 状态转换为（ ）。', '[''CLOSE_WAIT'', ''TIME_WAIT'', ''FIN_WAIT_1'', ''FIN_WAIT_2'']', "['B']", 'B
本题考察四次挥手的状态转换。
当客户机收到服务器发送的 FIN 段并向服务器发送 ACK 段后，客户机的 TCP 状态变为 TIME_WAIT，此时 TCP 连接还未释放，必须经过时间等待计时器设置的时间 2MSL（最长报文段寿命）后，客户机才进入 CLOSED（连接关闭）状态。', 9, 'Mogullzr', '2026-01-29 18:55:36', 9, NULL, '2026-01-29 18:55:36', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1312', '3901');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3902, '2021年408考研真题第39题', '简单', '2021年408真题', '若大小为 12B 的应用层数据分别通过 1 个 UDP 数据报和 1 个 TCP 段传输，则该 UDP 数据报 和 TCP 段实现的有效载荷（应用层数据）最大传输效率分别是（ ）。', '[''37.5%, 16.7%'', ''37.5%, 37.5%'', ''60.0%, 16.7%'', ''60.0%, 37.5%'']', "['D']", 'D
应用层数据交给传输层时，放在报文段的数据部分。UDP 首部有 8B，TCP 首部最短有 20B。为了达到最大传输效率，通过 UDP 传输时，总长度为 20B，最大传输效率是 12B/20B=60% 通过 TCP 传输时，总长度为 32B，最大传输效率是 12B/32B=37.5%。', 9, 'Mogullzr', '2026-01-29 18:55:36', 9, NULL, '2026-01-29 18:55:36', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/209844d8a93d4e42a217c67d7695e4f1.jpg)
', '[''501 ~ 1000'', ''601 ~ 1100'', ''701 ~ 1000'', ''801 ~ 1100'']', "['C']", 'C
本题考察 TCP 的滑动窗口机制。
依题意，甲发送完 200B 报文后，继续发送的报文段中序号字段 seq=701。由于乙告知接收窗口为 500，且甲未收到乙对 seq =501 报文段的确认，那么甲还能发送的报文段字节数为 500-200=300B，因此甲在未收到新的确认段之前，还能发送的数据序号范围是 701~1000。', 9, 'Mogullzr', '2026-01-29 18:55:38', 9, NULL, '2026-01-29 18:55:38', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1126', '3903');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3904, '2021年408考研真题第41题', '简单', '2021年408真题', '已知无向连通图 G 由顶点集 V 和边集 E 组成，|E| > 0，当 G 中度为奇数的顶点个数为不大于 2 的偶数时，G 存在包含所有边且长度为|E|的路径（称为 EL 路径）。设图 G 采用邻接矩阵存储，类型定义如下：typedefstruct{// 图的定义intnumVertices,numEdges;// 图中实际的顶点数和边数charVerticesList[MAXV];// 顶点表，MAXV 为已定义常量intEdge[MAXV][MAXV];// 邻接矩阵}MGraph;请设计算法 int IsExistEL(MGraph G)，判断 G 是否存在 EL 路径，若存在，则返回 1，否则返回 0。要求：(1) 给出算法的基本设计思想。(2) 根据设计思想，采用 C 或 C++ 语言描述算法，关键之处给出注释。(3) 说明你所设计算法的时间复杂度和空间复杂度。', '[]', '', '1）算法的基本设计思想
本算法题属于送分题，题干已经告诉我们算法的思想。对于采用邻接矩阵存储的无向
图，在邻接矩阵的每一行（列）中，非零元素的个数为本行（列）对应顶点的度。可
以依次计算连通图 G 中各顶点的度，并记录度为奇数的顶点个数，若个数为 0 或 2，则
返回 1，否则返回 0。
2）算法实现
intisExistEL(MGraphG){// 统计度为奇数的定点个数intcount=0;for(intv=0;v<numVertices;v++){// 该顶点的度intdegree=0;for(inte=0;e<numEdges;e++){if(Edge[v][e]==1){degree++;}}if(degree%2==1){count++;}}if(count==0||count==2){return1;}return0;}
3）时间和空间复杂度
算法需要遍历整个矩阵，所以时间复杂度为$O(n^{2})$，空间复杂度为$O(1)$。
', 9, 'Mogullzr', '2026-01-29 18:55:38', 9, NULL, '2026-01-29 18:55:38', 1, 0);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1193', '3904');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3905, '2021年408考研真题第42题', '简单', '2021年408真题', '已知某排序算法如下：voidcmpCountSort(inta[],intb[],intn){inti,j,*count;count=(int*)malloc(sizeof(int)*n)//C++ 语言：count = new int[n];for(i=0;i<n;i++)count[i]=0;for(i=0;i<n-1;i++)for(j=i+1;j<n;j++)if(a[i]<a[j])count[j]++;elsecount[i]++;for(i=0;i<n;i++)b[count[i]]=a[i];free(count);// C++ 语言：delete count;}请回答下列问题。(1) 若有 int a[] = {25, -10, 25, 10, 11, 19}, b[6]; ，则调用 cmpCountSort(a, b, 6) 后数组 b 中的内容是什么？(2) 若 a 中含有 n 个元素，则算法执行过程中，元素之间的比较次数是多少？(3) 该算法是稳定的吗？若是，则阐述理由；否则，修改为稳定排序算法。', '[]', '', 'cmpCountSort 算法基于计数排序的思想，对序列进行排序。cmpCountSort 算法遍历数组
...
2）由代码for(i=0;i<n-1;i++)和for(j=i+1;j<n;j++)可知，在循环过程中，每
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/0d33e6978f8b434396f0ca939e12f969.svg)
...
', 9, 'Mogullzr', '2026-01-29 18:55:39', 9, NULL, '2026-01-29 18:55:39', 1, 0);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3908, '2021年408考研真题第45题', '简单', '2021年408真题', '下表给出了整型信号量 S 的 wait() 和 signal() 操作的功能描述，以及采用开/关中断指令实现信号量操作互斥的两种方法。请回答下列问题。(1) 为什么在 wait() 和 signal() 操作中对信号量 S 的访问必须互斥执行？(2) 分别说明方法 1 和方法 2 是否正确。若不正确，请说明理由。(3) 用户程序能否使用开/关中断指令实现临界区互斥？为什么？', '[]', '', '1）信号量$S$是能被多个进程共享的变量，多个进程都可通过 wait() 和 signal() 对$S$进行读、写操作。所以，wait() 和 signal() 操作中对 S 的访问必须是互斥的。
...
', 9, 'Mogullzr', '2026-01-29 18:55:40', 9, NULL, '2026-01-29 18:55:40', 1, 0);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/345653400ed84d1aae06b7d0af734369.svg)
...
', 9, 'Mogullzr', '2026-01-29 18:55:41', 9, NULL, '2026-01-29 18:55:41', 1, 0);
