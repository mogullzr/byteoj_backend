...
$$
则$T(n) = T(n - 1) + 1 = T(n - 2) + 2 = ⋯ = T(1) + n - 1 = O(n)$，故时间复杂度为$O(n)$。', 9, 'Mogullzr', '2026-02-01 23:08:08', 9, NULL, '2026-02-01 23:08:08', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1173', '3441');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3442, '2012年408考研真题第2题', '简单', '2012年408真题', '已知操作符包括`+`、`−`、`∗`、`/`、`(`和`)`。将中缀表达式`a+b−a∗((c+d)/e−f)+g`转换为等价的后缀表达式`ab+acd+e/f−∗−g+`时，用栈来存放暂时还不能确定运算次序的操作符，若栈初始为空，则转换过程中同时保存在栈中的操作符的最大个数是（ ）。', '[''5'', ''7'', ''8'', ''11'']', "['A']", 'A</font></strong>
表达式求值是栈的典型应用。中缀表达式不仅依赖千运算符的优先级，而且要处理括号。后缀表达式的运算符在表达式的后面且没有括号，其形式已经包含了运算符的优先级。所以从中序转后序需要用运算符进行处理，使其包含运算符优先级的信息，从而转换为后缀表达式的形式。', 9, 'Mogullzr', '2026-02-01 23:08:08', 9, NULL, '2026-02-01 23:08:08', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1198', '3442');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3443, '2012年408考研真题第3题', '简单', '2012年408真题', '若一棵二叉树的前序遍历序列为 a, e, b, d, c，后序遍历序列为 b, c, d, e, a，则根结点的孩子结点（ ）。', '[''只有 e'', ''有 e、b'', ''有 e、c'', ''无法确定'']', "['A']", 'A</font></strong>
本题考察双序列组合重建二叉树，前序序列和后序序列不能唯一确定一棵二叉树，但可以确定二叉树中结点的祖先关系：当两个结点的前序序列为 XY 与后序序列为 YX 时，则 X 为 Y 的祖先。考虑前序序列 a,e,b,d,c、后序序列 b,c,d,e,a 且，可知 a 为根结点，e 为 a 的孩子结点。此外，a 的孩子结点的前序序列 e,b,d,c、后序序列 b,c,d,e，可知 e 是 bcd 的祖先，故根结点的孩子结点只有 e。故选 A。', 9, 'Mogullzr', '2026-02-01 23:08:08', 9, NULL, '2026-02-01 23:08:08', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1089', '3443');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3444, '2012年408考研真题第4题', '简单', '2012年408真题', '若平衡二叉树的高度为 6 ，且所有非叶结点的平衡因子均为 1 ，则该平衡二叉树的结点总数为（ ）。', '[''10'', ''20'', ''32'', ''33'']', "['B']", 'B</font></strong>
所有非叶结点的平衡因子均为 1，即平衡二叉树满足平衡的最少结点情况，如下图所示。
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/8e25a3eb00d14453ac96a079f86c4575.jpg)
对于高度为 N、左右子树的高度分别为 N-1 和 N-2、所有非叶结点的平衡因子均为 1 的平衡二叉树，总结点数的公式为：$C_{N} = C_{N-1} + C_{N-2} + 1$,$C_{1} = 1$,$C_{2} = 2$,$C_{3} = 2 + 1 + 1 = 4$，可推出$C_{6} = 20$。', 9, 'Mogullzr', '2026-02-01 23:08:09', 9, NULL, '2026-02-01 23:08:09', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1090', '3444');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3445, '2012年408考研真题第5题', '简单', '2012年408真题', '对有 n 个结点、e 条边且使用邻接表存储的有向图进行广度优先遍历，其算法时间复杂度是（）。', '[''$O(n)$'', ''$O(e)$'', ''$O(n + e)$'', ''$O(n × e)$'']', "['C']", 'C</font></strong>
广度优先遍历需要借助队列实现。邻接表的结构包括：顶点表；边表（有向图为出边表）。当采用邻接表存储方式时，在对图进行广度优先遍历时每个顶点均需入队一次（顶点表遍历），故时间复杂度为$O(n)$，在搜索所有顶点的邻接点的过程中，每条边至少访问一次（出边表遍历），故时间复杂度为$O(e)$，算法总的时间复杂度为$O(n + e)$。', 9, 'Mogullzr', '2026-02-01 23:08:11', 9, NULL, '2026-02-01 23:08:11', 1, 1);
...
\\end{vmatrix}
$$
则存在两个拓扑序列，因此该图存在可能不唯一的拓扑序列。结论：对于任一有向图，如果它的邻接矩阵中对角线以下（或以上）的元素均为零，则存在拓扑序列（可能不唯一）。反正，若图存在拓扑序列，却不一定能满足邻接矩阵中主对角线以下的元素均为零，但是可以通过适当地调整结点编号，使其邻接矩阵满足前述性质。', 9, 'Mogullzr', '2026-02-01 23:08:11', 9, NULL, '2026-02-01 23:08:11', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/cf855f4461cf49d59b5d354b741fddc5.svg)
...
后续目标顶点依次为 f, d, e。【排除法】对于 A，若下一个顶点为 d，路径 a,b,d 的长度5，而 a,b,c,f 的长度仅为4，显然错误。同理可以排除B。将 f 加入集合 S 后，采用上述的方法也可以排除 D。', 9, 'Mogullzr', '2026-02-01 23:08:12', 9, NULL, '2026-02-01 23:08:12', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1129', '3447');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3448, '2012年408考研真题第8题', '简单', '2012年408真题', '下列关于最小生成树的叙述中，正确的是（）。Ⅰ．最小生成树的代价唯一Ⅱ．所有权值最小的边一定会出现在所有的最小生成树中Ⅲ．使用普里姆（Prim）算法从不同顶点开始得到的最小生成树一定相同Ⅳ．使用普里姆算法和克鲁斯卡尔（Kruskal）算法得到的最小生成树总不相同', '[''仅Ⅰ'', ''仅Ⅱ'', ''仅Ⅰ、Ⅲ'', ''仅Ⅱ、Ⅳ'']', "['A']", 'A</font></strong>
对于 I，最小生成树的树形可能不唯一（这是因为可能存在权值相同的边），但是代价一定是唯一的，I 正确。对于Ⅱ，如果权值最小的边有多条并且构成环状，则总有权值最小的边将不出现在某棵最小生成树中，Ⅱ错误。对于Ⅲ，设 N 个结点构成环，N-1 条边权值相等，则从不同的顶点开始普里姆算法会得到 N-1 中不同的最小生成树，Ⅱ 错误。对于 V，当最小生成树唯一时（各边的权值不同），Prim 算法和 Kruskal 算法得到的最小生成树相同，IV 错误。', 9, 'Mogullzr', '2026-02-01 23:08:12', 9, NULL, '2026-02-01 23:08:12', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/1618927905984fdd96fe8585d85db48f.svg)
...
- 对于 V，二路归并排序每趟对子表进行两两归并从而得到若干个局部有序的结果，但无法确定最终位置。I、III、IV 正确，本题选择 A。', 9, 'Mogullzr', '2026-02-01 23:08:15', 9, NULL, '2026-02-01 23:08:15', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1096', '3450');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3451, '2012年408考研真题第11题', '简单', '2012年408真题', '对同一待排序序列分别进行折半插入排序和直接插入排序，两者之间可能的不同之处是（ ）。', '[''排序的总趟数'', ''元素的移动次数'', ''使用辅助空间的数量'', ''元素之间的比较次数'']', "['D']", 'D</font></strong>
本题考察插入排序，折半插入排序与直接插入排序是将待插入元素插入前面的有序子表，区别是：确定当前记录在前面有序子表中的位置时，直接插入排序是采用顺序查找法，而折半插入排序是采用折半查找法。排序的总趟数取决于元素个数$n$，两者都是$n - 1$趟。元素的移动次数都取决于初试序列，两者相同。使用辅助空间的数量也都是 O(1)。折半插入排序的比较次数与序列初态无关，为$O(nlog_{2}n)$：直接插入排序的比较次数与序列初态有关，为$O(n) ∼ O(n^{2})$。', 9, 'Mogullzr', '2026-02-01 23:08:15', 9, NULL, '2026-02-01 23:08:15', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1096', '3451');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3452, '2012年408考研真题第12题', '简单', '2012年408真题', '假定基准程序 A 在某计算机上的运行时间为 100 秒，其中 90 秒为 CPU 时间，其余为 I/O 时间。若 CPU 速度提高 50%，I/O 速度不变，则运行基准程序 A 所耗费的时间是（ ）。', '[''55s'', ''60s'', ''65s'', ''70s'']', "['D']", 'D</font></strong>
程序 A 的运行时间为 100s，除去 CPU 时间 90s，剩余 10s 为 I/O 时间。CPU 提速后运行基准程序 A 所耗费的时间是 T = 90/1.5 + 10 = 70s。【误区】CPU速度提高50%，则CPU时间减少一半，而误选A。', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
...
unsigned int y=x;
```
得到 y 的机器数为（ ）。', '[''0000 7FFAH'', ''0000 FFFAH'', ''FFFF 7FFAH'', ''FFFF FFFAH'']', "['B']", 'B</font></strong>
将一个 16 位 unsigned short 转换成 32 位形式的 unsigned int，因为都是无符号数，新表示形式的高位用 0 填充。16 位无符号整数所能表示的最大值为 65 5 35，其十六进制表示为 FFFFH，故 x 的十六进制表示为 FFFFH - 5H = FFFAH，所以 y 的十六进制表示为 0000 FFFAH。', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3454, '2012年408考研真题第14题', '简单', '2012年408真题', 'float 类型（即 IEEE754 单精度浮点数格式）能表示的最大正整数是（ ）。', '[''$2^{126} - 2^{103}$'', ''$2^{127} - 2^{104}$'', ''$2^{127} - 2^{103}$'', ''$2^{128} - 2^{104}$'']', "['D']", 'D</font></strong>
IEEE754 单精度浮点数是尾数用采取隐藏位策略的原码表示，且阶码用移码（偏置值为$127$）表示的浮点数。规格化的短浮点数的真值为：$(−1)S × 1.m × 2^{E-127}$，$S$为符号位，阶码$E$的取值为$1 ∼ 254$（8 位表示），尾数$m$为$23$位，共$32$位；故 float 类型能表示的最大整数是$1.111 ⋯ 1 × 2^{254-127} = 2^{127} × (2 - 2^{−23}) = 2^{128} - 2^{104}$，故选 D。', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
...
record.a=273;
```
若 record 变量的首地址为 0xC008，则地址 0xC008 中内容及 record.c 的地址分别为（ ）。', '[''0x00、0xC00D'', ''0x00、0xC00E'', ''0x11、0xC00D'', ''0x11、0xC00E'']', "['D']", 'D</font></strong>
...
', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1203', '3455');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3456, '2012年408考研真题第16题', '简单', '2012年408真题', '下列关于闪存（Flash Memory）的叙述中，错误的是（ ）。', '[''信息可读可写，并且读、写速度一样快'', ''存储元由 MOS 管组成，是一种半导体存储器'', ''掉电后信息不丢失，是一种非易失性存储器'', ''采用随机访问方式，可替代计算机外部存储器'']', "['A']", 'A</font></strong>
闪存是 EEPROM 的进一步发展，可读可写，用 MOS 管的浮栅上有无电荷来存储信息。闪存依然是 ROM 的一种，写入时必须先擦除原有数据，故写的速度比读的速度要慢不少（硬件常识）。闪存是一种非易失性存储器，它采用随机访问方式。现在常见的 SSD 固态硬盘，即由Flash 芯片组成。', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3457, '2012年408考研真题第17题', '简单', '2012年408真题', '假设某计算机按字编址，Cache 有 4 个行，Cache 和主存之间交换的块大小为 1 个字。若 Cache 的内容初始为空，采用 2 路组相联映射方式和 LRU 替换策略。访问的主存地址依次为 0,4,8,2,0,6,8,6,4,8 时，命中 Cache 的次数是（ ）。', '[''1'', ''2'', ''3'', ''4'']', "['C']", 'C</font></strong>
...
| 第1组 | 块2 |  |  |  |  |  | 2 | 2 | 2 | 2 | 2 |
|  | 块3 |  |  |  | 2 | 2 | 6 | 6 | 6\\* | 6 | 6 |
注：“_” 表示当前访问块，“*” 表示本次访问命中。注意：在不同的《计算机组成原理》教材中，关于组相联映射的介绍并不相同。通常采用唐朔飞教材中的方式，但本题中采用的是蒋本珊教材中的方式。可以推断两次命题的老师应该不是同一位老师，这也给考生答题带来了困扰。', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1101', '3457');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3458, '2012年408考研真题第18题', '简单', '2012年408真题', '某计算机的控制器采用微程序控制方式，微指令中的操作控制字段采用字段直接编码法，共有 33 个微命令，构成 5 个互斥类，分别包含 7、3、12、5 和 6 个微命令，则操作控制字段至少有（ ）。', '[''5 位'', ''6 位'', ''15 位'', ''33 位'']', "['C']", 'C</font></strong>
字段直接编码方式将微命令字段分成若干个小字段，互斥性微命令组合在同一字段中，相容性微命令分在不同字段中，每个字段还要留出一个状态，表示本字段不发出任何微命令。5 个互斥类分别包含 7、3、12、5 和 6 个微命令，需要 3、2、4、3 和 3 位，共 15 位。', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1105', '3458');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3459, '2012年408考研真题第19题', '简单', '2012年408真题', '某同步总线的时钟频率为 100MHz，宽度为 32 位，地址/数据线复用，每传输一个地址或数据占用一个时钟周期。若该总线支持突发（猝发）传输方式，则一次 “主存写” 总线事务传输 128 位数据所需要的时间至少是（ ）。', '[''20ns'', ''40ns'', ''50ns'', ''80ns'']', "['C']", 'C</font></strong>
总线频率为 100 MHz，则时钟周期为 10ns。总线位宽与存储字长都是 32 位，故每一个时钟周期可传送一个 32 位存储字。猝发式发送可以连续传送地址连续的数据，故总的传送时间为：传送地址 10ns，传送 128 位数据 40ns，共需 50ns。', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1204', '3459');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3460, '2012年408考研真题第20题', '简单', '2012年408真题', '下列关于 USB 总线特性的描述中，错误的是（ ）。', '[''可实现外设的即插即用和热插拔'', ''可通过级联方式连接多台外设'', ''是一种通信总线，可连接不同外设'', ''同时可传输 2 位数据，数据传输率高'']', "['D']", 'D</font></strong>
USB（通用串行总线）的特点有：① 即插即用；② 热插拔；③ 有很强的连接能力，采用菊花链形式将众多外设连接起来；④ 有很好的可扩充性，一个 USB 控制器可扩充高达 127 个外部USB 设备；⑤ 高速传输，速度可达 480Mbps。所以 A、B、C 选项都符合 USB 总线的特点。对于 D, USB 是串行总线，不能同时传输 2 位数据。', 9, 'Mogullzr', '2026-02-01 23:08:16', 9, NULL, '2026-02-01 23:08:16', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1152', '3460');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3461, '2012年408考研真题第21题', '简单', '2012年408真题', '下列选项中，在 I/O 总线的数据线上传输的信息包括（ ）。Ⅰ. I/O 接口中的命令字Ⅱ. I/O 接口中的状态字Ⅲ. 中断类型号', '[''仅Ⅰ、Ⅱ'', ''仅Ⅰ、Ⅲ'', ''仅Ⅱ、Ⅲ'', ''Ⅰ、Ⅱ、Ⅲ'']', "['D']", 'D</font></strong>
根据总线组成，I/O 接口与 CPU 之间的 I/O 总线有数据线、控制线和地址线。控制线和地址线都是单向传输的，从 CPU 传送给 I/O 接口，而 I/O 接口中的命令字、状态字以及中断类型号均是由 I/O 接口发往 CPU 的，故只能通过 I/O 总线的数据线传输。', 9, 'Mogullzr', '2026-02-01 23:08:17', 9, NULL, '2026-02-01 23:08:17', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1179', '3461');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3462, '2012年408考研真题第22题', '简单', '2012年408真题', '响应外部中断的过程中，中断隐指令完成的操作，除保护断点外，还包括（ ）。Ⅰ. 关中断Ⅱ. 保存通用寄存器的内容Ⅲ. 形成中断服务程序入口地址并送 PC', '[''仅Ⅰ、Ⅱ'', ''仅Ⅰ、Ⅲ'', ''仅Ⅱ、Ⅲ'', ''Ⅰ、Ⅱ、Ⅲ'']', "['B']", 'B</font></strong>
在响应外部中断的过程中，中断隐指令完成的操作包括：①关中断；②保护断点；③引出中断服务程序（形成中断服务程序入口地址并送 PC）,所以只有 I、III 正确。Ⅱ 中的保存通用寄存器的内容是在进入中断服务程序后首先进行的操作。', 9, 'Mogullzr', '2026-02-01 23:08:17', 9, NULL, '2026-02-01 23:08:17', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3463, '2012年408考研真题第23题', '简单', '2012年408真题', '下列选项中，不可能在用户态发生的事件是（ ）。', '[''系统调用'', ''外部中断'', ''进程切换'', ''缺页'']', "['C']", 'C</font></strong>
本题关键是对 “在用户态<strong><font color=''#2656b3''>发生</font></strong>”（与上题的 “<strong><font color=''#2656b3''>执行</font></strong>” 区分）的理解。对于 A，系统调用是操作系统提供给用户程序的接口，系统调用发生在用户态，被调用程序在核心态下执行。对于 B，外部中断是用户态到核心态的 “门”，也发生在用户态，在核心态完成中断过程。对于 C，进程切换属于系统调用执行过程中的事件，只能发生在核心态。对于 D，缺页产生后，在用户态发生缺页中断，然后进入核心态执行缺页中断服务程序。', 9, 'Mogullzr', '2026-02-01 23:08:17', 9, NULL, '2026-02-01 23:08:17', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1182', '3463');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3464, '2012年408考研真题第24题', '简单', '2012年408真题', '中断处理和子程序调用都需要压栈以保护现场，中断处理一定会保存而子程序调用不需要保存其内容的是（ ）。', '[''程序计数器'', ''程序状态字寄存器'', ''通用数据寄存器'', ''通用地址寄存器'']', "['B']", 'B</font></strong>
子程序调用只需保存程序断点，即该指令的下一条指令的地址；中断调用子程序不仅要保护断点（PC 的内容），而且要保护程序状态字寄存器的内容 PSW。在中断处理中，最重要的两个寄存器是 PC 和 PSWR。', 9, 'Mogullzr', '2026-02-01 23:08:17', 9, NULL, '2026-02-01 23:08:17', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1150', '3464');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3465, '2012年408考研真题第25题', '简单', '2012年408真题', '下列关于虚拟存储器的叙述中，正确的是（ ）。', '[''虚拟存储只能基于连续分配技术'', ''虚拟存储只能基于非连续分配技术'', ''虚拟存储容量只受外存容量的限制'', ''虚拟存储容量只受内存容量的限制'']', "['B']", 'B</font></strong>
在程序装入时，可以只将程序的一部分装入内存，而将其余部分留在外存，就可以自动程序执行。采用连续分配方式时，会使相当一部分内存空间都处于暂时或 “永久” 的空闲状态，造成内存资源的严重浪费，也无法从逻辑上扩大内存容量，因此虚拟内存的实现只能建立在离散分配的内存管理的基础上。有以下三种实现方式：①请求分页存储管理；②请求分段存储管理；③请求段页式存储管理。虚拟存储器容量既不受外存容量限制，也不受内存容量限制，而是由 CPU 的寻址范围决定的。', 9, 'Mogullzr', '2026-02-01 23:08:17', 9, NULL, '2026-02-01 23:08:17', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1138', '3465');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3466, '2012年408考研真题第26题', '简单', '2012年408真题', '用户程序发出磁盘 I/O 请求后，系统的正确处理流程是操作系统的 I/O 子系统通常由四个层次组成，每一层明确定义了与邻近层次的接口。其合理的层次组织排列顺序是（ ）。', '[''用户级 I/O 软件、设备无关软件、设备驱动程序、中断处理程序'', ''用户级 I/O 软件、设备无关软件、中断处理程序、设备驱动程序'', ''用户级 I/O 软件、设备驱动程序、设备无关软件、中断处理程序'', ''用户级 I/O 软件、中断处理程序、设备无关软件、设备驱动程序'']', "['A']", 'A</font></strong>
设备管理软件一般分为以下层次：用户层、与设备无关的系统调用处理层、设备驱动程序以及中断处理程序。', 9, 'Mogullzr', '2026-02-01 23:08:17', 9, NULL, '2026-02-01 23:08:17', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/b3426b47daba4202a64f93ab1d7cead9.jpg)
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/6aff3551bde9483a9a97cd75150df75c.jpg)
比较 Need 和 Available 可以发现，初始时进程$P_{1}$与$P_{3}$可满足需求，排除 A、C。尝试给$P_{1}$分配资源，则$P_{1}$完成后 Available 将变为 (6,3,6)，无法满足$P_{0}$的需求，排除 B。尝试给$P_{3}$分配资源，则$P_{3}$完成后 Available 将变为 (4,3,7)，该向量能满足其他所有进程的需求。所以，以$P_{3}$开头的所有序列都是安全序列。', 9, 'Mogullzr', '2026-02-01 23:08:19', 9, NULL, '2026-02-01 23:08:19', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1185', '3467');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3468, '2012年408考研真题第28题', '简单', '2012年408真题', '若一个用户进程通过 read 系统调用读取一个磁盘文件中的数据，则下列关于此过程的叙述中，正确的是（ ）。Ⅰ. 若该文件的数据不在内存中，则该进程进入睡眠等待状态Ⅱ. 请求 read 系统调用会导致 CPU 从用户态切换到核心态Ⅲ. read 系统调用的参数应包含文件的名称', '[''仅Ⅰ、Ⅱ'', ''仅Ⅰ、Ⅲ'', ''仅Ⅱ、Ⅲ'', ''Ⅰ、Ⅱ和Ⅲ'']', "['A']", 'A</font></strong>
对于 I，当所读文件的数据不在内存时，产生中断（缺页中断），原进程进入阻塞状态，直到所需数据从外存调入内存后，才将该进程唤醒。对于 II，read 系统调用通过陷入将 CPU 从用户态切换到核心态，从而获取操作系统提供的服务。对于Ⅲ，要读一个文件首先要用 open 系统调用打开该文件。open 中的参数包含文件的路径名与文件名，而 read 只需要使用 open 返回的文件描述符，并不使用文件名作为参数。read 要求用户提供三个输入参数：①文件描述符 fd；②buf 缓冲区首址；③传送的字节数 n。read 的功能是试图从 fd 所指示的文件中读入 n 个字节的数据，并将它们送至由指针 buf 所指示的缓冲区中。', 9, 'Mogullzr', '2026-02-01 23:08:21', 9, NULL, '2026-02-01 23:08:21', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3469, '2012年408考研真题第29题', '简单', '2012年408真题', '一个多道批处理系统中仅有$P_{1}$和$P_{2}$两个作业，$P_{2}$比$P_{1}$晚 5ms 到达，它们的计算和 I/O 操作顺序如下：$P_{1}$：计算 60ms，I/O 80ms，计算 20ms$P_{2}$：计算 120ms，I/O 40ms，计算 40ms若不考虑调度和切换时间，则完成两个作业需要的时间最少是（ ）。', '[''240ms'', ''260ms'', ''340ms'', ''360ms'']', "['B']", 'B</font></strong>
由于$P_{2}$比$P_{1}$晚 5ms 到达，$P_{1}$先占用 CPU, 作业运行的甘特图如下所示。
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/2e6f1cb10ad1480280ea1f0f8785a89b.jpg)
', 9, 'Mogullzr', '2026-02-01 23:08:21', 9, NULL, '2026-02-01 23:08:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1111', '3469');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3470, '2012年408考研真题第30题', '简单', '2012年408真题', '若某单处理器多进程系统中有多个就绪态进程，则下列关于处理机调度的叙述中，错误的是（ ）。', '[''在进程结束时能进行处理机调度'', ''创建新进程后能进行处理机调度'', ''在进程处于临界区时不能进行处理机调度'', ''在系统调用完成并返回用户态时能进行处理机调度'']', "['C']", 'C</font></strong>
选项 A、B、D 显然是可以进行处理机调度的情况。对于 C，当进程处于临界区时，说明进程正在占用处理机，只要不破坏临界资源的使用规侧，是不会影响处理机调度的。比如，通常访问的临界资源可能是慢速的外设（如打印机），如果在进程访问打印机时，不能进行处理机调度，那么系统的性能将是非常差的。', 9, 'Mogullzr', '2026-02-01 23:08:23', 9, NULL, '2026-02-01 23:08:23', 1, 1);
...
在引入线程后，进程依然还是资源分配的基本单位，线程是调度的基本单位，同一进程中的各个线程共享进程的地址空间。在用户级线程中，有关线程管理的所有工作都由应用程序完成，无须内核的干预，内核意识不到线程的存在。', 9, 'Mogullzr', '2026-02-01 23:08:23', 9, NULL, '2026-02-01 23:08:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1183', '3471');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3472, '2012年408考研真题第32题', '简单', '2012年408真题', '下列选项中，不能改善磁盘设备 I/O 性能的是（ ）。', '[''重排 I/O 请求次序'', ''在一个磁盘上设置多个分区'', ''预读和滞后写'', ''优化文件物理块的分布'']', "['B']", 'B</font></strong>
对于 A，重排 I/O 请求次序也就是进行I/O 调度，从而使进程之间公平地共享磁盘访问，减少 I/O 完成所需要的平均等待时间。对于 C，缓冲区结合预读和滞后写技术对于具有重复性及阵发性的 I/O 进程改善磁盘 I/O 性能很有帮助。对于 D，优化文件物理块的分布可以减少寻找时间与延迟时间，从而提高磁盘性能。在一个磁盘上设置多个分区与改善设备/O 性能并无多大联系，相反还会带来处理的复杂和降低利用率。', 9, 'Mogullzr', '2026-02-01 23:08:23', 9, NULL, '2026-02-01 23:08:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1207', '3472');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3473, '2012年408考研真题第33题', '简单', '2012年408真题', '在 TCP/IP 体系结构中，直接为 ICMP 提供服务的协议是（ ）。', '[''PPP'', ''IP'', ''UDP'', ''TCP'']', "['B']", 'B</font></strong>
ICMP 报文作为数据字段封装在 IP 分组中，因此，IP 协议直接为 ICMP 提供服务。UDP 和 TCP 都是传输层协议，为应用层提供服务。PPP 协议是链路层协议，为网络层提供服务。', 9, 'Mogullzr', '2026-02-01 23:08:23', 9, NULL, '2026-02-01 23:08:23', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3474, '2012年408考研真题第34题', '简单', '2012年408真题', '在物理层接口特性中，用于描述完成每种功能的事件发生顺序的是（ ）。', '[''机械特性'', ''功能特性'', ''过程特性'', ''电气特性'']', "['C']", 'C</font></strong>
此题考查物理层接口特性，过程特性定义各条物理线路的工作过程和时序关系。', 9, 'Mogullzr', '2026-02-01 23:08:23', 9, NULL, '2026-02-01 23:08:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1208', '3474');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3475, '2012年408考研真题第35题', '简单', '2012年408真题', '以太网的 MAC 协议提供的是（ ）。', '[''无连接不可靠服务'', ''无连接可靠服务'', ''有连接不可靠服务'', ''有连接可靠服务'']', "['A']", 'A</font></strong>
考虑到局域网信道质量好，以太网采取了两项重要的措施以使通信更简便：① 采用无连接的工作方式；② 不对发送的数据帧进行编号，也不要求对方发回确认。因此，以太网提供的服务是不可靠的服务，即尽最大努力的交付。差错的纠正由高层完成。', 9, 'Mogullzr', '2026-02-01 23:08:23', 9, NULL, '2026-02-01 23:08:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1209', '3475');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3476, '2012年408考研真题第36题', '简单', '2012年408真题', '两台主机之间的数据链路层采用后退 N 帧协议 (GBN) 传输数据，数据传输速率为 16 kbps，单向传播时延为 270 ms，数据帧长度范围是 128~512 字节，接收方总是以与数据帧等长的帧进行确认。为使信道利用率达到最高，帧序号的比特数至少为（ ）。', '[''5'', ''4'', ''3'', ''2'']', "['B']", 'B</font></strong>
本题考察回退 N 帧的极限，即求从发送一个帧到接收到这个帧的确认为止的时间内最多可以发送多少数据帧。要尽可能多发帧，应以短的数据帧计算，首先计算出发送一帧的时间：$128 × 8/(16 × 10^{3}) = 64$ms；发送一帧到收到确认为止的总时间：$64 + 270 × 2 + 64 = 668$ms；这段时间总共可以发送$668/64 = 10.4$（帧），发送这么多帧至少需要用$4$位比特进行编号。', 9, 'Mogullzr', '2026-02-01 23:08:23', 9, NULL, '2026-02-01 23:08:23', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3477, '2012年408考研真题第37题', '简单', '2012年408真题', '下列关于 IP 路由器功能的描述中，正确的是（ ）。I. 运行路由协议，设置路由表II. 监测到拥塞时，合理丢弃 IP 分组III. 对收到的 IP 分组头进行差错校验，确保传输的 IP 分组不丢失IV. 根据收到的 IP 分组的目的 IP 地址，将其转发到合适的输出线路上', '[''仅 III、IV'', ''仅 I、II、III'', ''仅 I、II、IV'', ''I、II、III、IV'']', "['C']", 'C</font></strong>
I 和 V 显然是 IP 路由器的功能。对于Ⅱ，当路由器监测到拥塞时，可合理丢弃 IP 分组，并向发出该 IP 分组的源主机发送一个源点抑制的 ICMP 报文。对于 III，路由器对收到的 IP 分组首部进行差错检验，丢弃有差错首部的报文，但不保证 IP 分组不丢失。对于 IV，这表述的也就是路由器的路由功能。', 9, 'Mogullzr', '2026-02-01 23:08:24', 9, NULL, '2026-02-01 23:08:24', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1139', '3477');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3478, '2012年408考研真题第38题', '简单', '2012年408真题', 'ARP 协议的功能是（ ）。', '[''根据 IP 地址查询 MAC 地址'', ''根据 MAC 地址查询 IP 地址'', ''根据域名查询\\xa0IP 地址'', ''根据 IP 地址查询域名'']', "['A']", 'A</font></strong>
在实际网络的数据链路层上传送数据时，最终必须使用硬件地址，ARP 协议是将网络层的 IP 地址解析为数据链路层的 MAC 地址。', 9, 'Mogullzr', '2026-02-01 23:08:24', 9, NULL, '2026-02-01 23:08:24', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1196', '3478');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3479, '2012年408考研真题第39题', '简单', '2012年408真题', '某主机的 IP 地址为 180.80.77.55，子网掩码为 255.255.252.0。若该主机向其所在子网发送广播分组，则目的地址可以是（ ）。', '[''180.80.76.0'', ''180.80.76.255'', ''180.80.77.255'', ''180.80.79.255'']', "['D']", 'D</font></strong>
子网掩码的第 3 个字节为 11111100，可知前 22 位为子网号、后 10 位为主机号。IP 地址的第 3 个字节为01001101（下画线为子网号的一部分），将主机号（即后 10 位）全置为 1，可以得到广播地址为 180.80.79.255。', 9, 'Mogullzr', '2026-02-01 23:08:24', 9, NULL, '2026-02-01 23:08:24', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/58246f6b75984ff48c32cc5628248b4a.svg)
', '[''SMTP、SMTP、SMTP'', ''POP3、SMTP、POP3'', ''POP3、SMTP、SMTP'', ''SMTP、SMTP、POP3'']', "['D']", 'D</font></strong>
SMTP采用 “推” 的通信方式，在用户代理向邮件服务器及邮件服务器之间发送邮件时，SMTP 客户主动将邮件“推”送到 SMTP 服务器。而POP3采用 “拉” 的通信方式，当用户读取邮件时，用户代理向邮件服务器发出请求，“拉”取用户邮箱中的邮件。', 9, 'Mogullzr', '2026-02-01 23:08:25', 9, NULL, '2026-02-01 23:08:25', 1, 1);
...
', 9, 'Mogullzr', '2026-02-01 23:08:25', 9, NULL, '2026-02-01 23:08:25', 1, 0);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/9211ad8dbc1c430fb8eaa60dd800c7fa.svg)
...
', 9, 'Mogullzr', '2026-02-01 23:08:27', 9, NULL, '2026-02-01 23:08:27', 1, 0);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/ef2ffb12b5204b929b446625aa0f67b4.jpg)
(1) 若 int 型变量 x 的值为 -513，存放在寄存器 R1 中，则执行指令 “SHR R1” 后，R1 的内容是多少（用十六进制表示）？(2) 若某个时间段中，有连续的 4 条指令进入流水线，在其执行过程中没有发生任何阻塞，则执行这 4 条指令所需的时钟周期数为多少？(3) 若高级语言程序中某赋值语句为`x=a+b`，`x`、`a`和`b`均为 int 型变量，它们的存储单元地址分别表示为`[x]`、`[a]`和`[b]`。该语句对应的指令序列及其在指令流水线中的执行过程如下图所示。则这 4 条指令执行过程中，$I_{3}$的 ID 段和$I_{4}$的 IF 段被阻塞的原因各是什么？(4) 若高级语言程序中某赋值语句为`x=x*2+a`，`x`和`a`均为 unsigned int 类型变量，它们的存储单元地址分别表示为`[x]`、`[a]`，则执行这条语句至少需要多少个时钟周期？要求模仿题 44 图画出这条语句对应的指令序列及其在流水线中的执行过程示意图。', '[]', '', '1）x 的机器码为$\\lbrack x \\rbrack 补​$= 1111 1101 1111 1111B，即指令执行前 (R1)=FDFFH，右移 1 位后为 1111 1110 1111 1111B，即指令执行后 (R1) = FEFFH。（2 分）
...
', 9, 'Mogullzr', '2026-02-01 23:08:29', 9, NULL, '2026-02-01 23:08:29', 1, 0);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/d3ac3053380d4df7bcf289c6e2cfd026.jpg)
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/2e816d1564a942b5ae249179058f6b84.svg)
...
', 9, 'Mogullzr', '2026-02-01 23:08:33', 9, NULL, '2026-02-01 23:08:33', 1, 0);
