...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/902056c0b77e4e06af09482fc688e492.svg)

', '[''1010H，1014H，1004H'', ''1010H，1004H，1014H'', ''1014H，1010H，1004H'', ''1014H，1004H，1010H'']', "['D']", 'D</font></strong>
根据存储状态，单链表的结构如下图所示。
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/58d93b3b3eb44a429b138846e4e0f536.svg)
其中“链接地址”是指结点 next 所指的内存地址。当结点 f 插入后，a 指向 f, f 指向 e, e 指向 b。显然 a、e 和 f 的“链接地址”分别是 f、b 和 e 的内存地址，即 1014H、1004H 和 1010H。', 9, 'Mogullzr', '2026-02-02 13:36:32', 9, NULL, '2026-02-02 13:36:32', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1130', '3629');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3630, '2016年408考研真题第2题', '简单', '2016年408真题', '已知一个带有表头结点的双向循环链表`L`，结点结构为`prev|data|next`，`prev`和`next`分别是指向其直接前驱和直接后继结点的指针。现要删除指针`p`所指的结点，正确的语句序列是（ ）。
', '[''`p->next->prev = p->prev; p->prev->next = p->prev; free(p);`'', ''`p->next->prev = p->next; p->prev->next = p->next; free(p);`'', ''`p->next->prev = p->next; p-> prev->next = p->prev; free(p);`'', ''`p->next->prev = p->prev; p->prev->next = p->next; free(p);`'']', "['D']", 'D</font></strong>
参考单链表删除操作，这种题为送分题。', 9, 'Mogullzr', '2026-02-02 13:36:33', 9, NULL, '2026-02-02 13:36:33', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/5188ac264f4345f781afad1611d62f93.svg)
...
满足题目中的“至少") 的队列。', 9, 'Mogullzr', '2026-02-02 13:36:35', 9, NULL, '2026-02-02 13:36:35', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1142', '3631');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3632, '2016年408考研真题第4题', '简单', '2016年408真题', '有一个 100 阶的三对角矩阵$M$，其元素$m_{i,j}(1 ≤ i ≤ 100, 1 ≤ j ≤ 100)$按行优先依次压缩存入下标从 0 开始的一维数组 N 中。元素$m_{30,30}$在数组 N 中的下标是（ ）。
', '[''86'', ''87'', ''88'', ''89'']', "['B']", 'B</font></strong>
参考三对角矩阵，采用压缩存储，将 3 条对角线上的元素按行优先方式存放在一 维数组 B 中，且$a_{1,1}$存放于 B[0] 中，其存储形式如下所示：
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/7d6fc12a4f75490792efc0a36104e719.jpg)
可以计算矩阵 A 中 3 条对角线上的元素$a_{i,j}$($1 ≤ i, j ≤ n, ∣i - j∣ ≤ 1$) 在一维数组 B 中存放的下标为$k = 2i + j - 3$。解法一：针对该题，仅需将数字逐一代入公式里面即可：k=2x30+30-3 = 87, 结果为 87。解法二：观察上图的三对角矩阵不难发现，第一行有两个元素，剩下的在元素$m_{30,30}$所在行之前的 28 行（注意下标$1 ≤ i ≤ 100$、$1 ≤ j ≤ 100$）中每行都有 3 个元素，而$m_{30,30}$之前仅有一个元素$m_{30,29}$, 那么不难发现元素$m_{30,30}$在数组 N 中的下标是：2+28x3+2-1 = 87。', 9, 'Mogullzr', '2026-02-02 13:36:35', 9, NULL, '2026-02-02 13:36:35', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3633, '2016年408考研真题第5题', '简单', '2016年408真题', '若森林 F 有 15 条边、25 个结点，则 F 包含树的个数是（ ）。
', '[''8'', ''9'', ''10'', ''11'']', "['C']", 'C</font></strong>
解法一：树有一个很重要的性质：在 n 个结点的树中有 n-1 条边，“那么对于每棵树，其结点数比边数多 1”。题中的森林中的结点数比边数多 10（即 25-15=10），显然共有 10 棵树。解法二：若考生再仔细分析可发现，此题也是考察图的某些方面的性质：生成树和生成森林。此时对于图的生成树有一个重要的性质：若图中顶点数为 n，则它的生成树含有 n-1 条边。对比解法一中树的性质，不难发现两种解法都利用到了“树中结点数比边数多 1”的性质，接下来的分析如解法一。', 9, 'Mogullzr', '2026-02-02 13:36:37', 9, NULL, '2026-02-02 13:36:37', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/c0a64fc126f34c36a0fc17a1c36362ac.jpg)
...
然后从$V_{2}$出发，访问与$V_{2}$邻接且未被访问的任一顶点（满足的只有$V_{5}$）, 按规则本应该访问$V_{5}$, 但选项 D 却访问$V_{3}$, 因此 D 错误。', 9, 'Mogullzr', '2026-02-02 13:36:38', 9, NULL, '2026-02-02 13:36:38', 1, 1);
...
根据拓扑排序的规则，输出每个顶点的同时还要删除以它为起点的边，这样对各顶点和边都要进行遍历，故拓扑排序的时间复杂度为$O(n + e)$。', 9, 'Mogullzr', '2026-02-02 13:36:38', 9, NULL, '2026-02-02 13:36:38', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/b06ad2b9a98a4030a56bc9da5c4300bd.svg)
...
', 9, 'Mogullzr', '2026-02-02 13:36:40', 9, NULL, '2026-02-02 13:36:40', 1, 1);
...
', '[''当 x 不在数组中'', ''当 x 接近数组开头处'', ''当 x 接近数组结尾处'', ''当 x 位于数组中间位置'']', "['B']", 'B</font></strong>
此题为送分题。该程序采用跳跃式的顺利查找法查找升序数组中的 X, 显然是 x 越靠前，比较次数才会越少。', 9, 'Mogullzr', '2026-02-02 13:36:40', 9, NULL, '2026-02-02 13:36:40', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1147', '3637');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3638, '2016年408考研真题第10题', '简单', '2016年408真题', 'B+ 树不同于 B 树的特点之一是（）
', '[''能支持顺序查找'', ''结点中含有关键字'', ''根结点至少有两个分支'', ''所有叶结点都在同一层上'']', "['A']", 'A</font></strong>
由于B+ 树的所有叶结点中包含了全部的关键字信息，且叶结点本身依关键字从小到大顺序
链接，可以进行顺序查找，而 B 树不支持顺序查找（只支持多路查找）。', 9, 'Mogullzr', '2026-02-02 13:36:40', 9, NULL, '2026-02-02 13:36:40', 1, 1);
...
', '[''希尔排序'', ''堆排序'', ''快速排序'', ''归并排序'']', "['D']", 'D</font></strong>
外部排序指待排序文件较大，内存一次性放不下，需存放在外部介质中。外部排序通常采用归并排序法。选项 A、B、C 都是内部排序的方法。', 9, 'Mogullzr', '2026-02-02 13:36:40', 9, NULL, '2026-02-02 13:36:40', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1264', '3639');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3640, '2016年408考研真题第12题', '简单', '2016年408真题', '将高级语言源程序转换为机器目标代码文件的程序是（ ）。
', '[''汇编程序'', ''链接程序'', ''编译程序'', ''解释程序'']', "['C']", 'C</font></strong>
翻译程序是指把高级语言源程序转换成机器语言程序（目标代码）的软件。翻译程序有两种：一种是编译程序，它将高级语言源程序一次全部翻译成目标程序，每次执行程序时，只要执行目标程序，因此，只要源程序不变，就无须重新编译。另一种是解释程序，它将源程序的一条语旬翻译成对应的机器目标代码，并立即执行，然后翻译下一条源程序语句并执行，直至所有源程序语句全部被翻译并执行完。所以解释程序的执行过程是翻译一句执行一句，并且不会生成目标程序。汇编程序也是一种语言翻译程序，它把汇编语言源程序翻译为机器语言程序。汇编语言是一种面向机器的低级语言，是机器语言的符号表示，与机器语言一一对应。', 9, 'Mogullzr', '2026-02-02 13:36:40', 9, NULL, '2026-02-02 13:36:40', 1, 1);
...
执行上述两条语句后，usi 的值为（ ）。
', '[''-32767'', ''32767'', ''32768'', ''32769'']', "['D']", 'D</font></strong>
结合题干及选项可知，short 为 16 位。因 C 语言中的数据在内存中为补码表示，si 对应的补码二进制表示为：1000 0000 0000 0001B, 最前面的一位 “1” 为符号位，表示负数，即 -32767。由 signed 型转化为等长 unsigned 型数据时，符号位成为数据的一部分，也就是说，负数转化为无符号数（即正数），其数值将发生变化。usi 对应的补码二进制表示与 si 的表示相同，但表示正数，为 32769。', 9, 'Mogullzr', '2026-02-02 13:36:40', 9, NULL, '2026-02-02 13:36:40', 1, 1);
...
', '[''22H'', ''33H'', ''66H'', ''77H'']', "['A']", 'A</font></strong>
...
| 内容 | 44H | 33H | 22H | 11H |
从而存储单元 0000 804 6H 中存放的是 22H。', 9, 'Mogullzr', '2026-02-02 13:36:40', 9, NULL, '2026-02-02 13:36:40', 1, 1);
...
', '[''1.25%'', ''2.5%'', ''12.5%'', ''25%'']', "['C']", 'C</font></strong>
...
的 7 次访问中全部命中，故该程序段执行过程中访问数组 a 的 Cache 缺失率约为 1/8（即 12.5%）。', 9, 'Mogullzr', '2026-02-02 13:36:40', 9, NULL, '2026-02-02 13:36:40', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3644, '2016年408考研真题第16题', '简单', '2016年408真题', '某存储器容量为 64KB，按字节编址，地址 4000H～5FFFH 为 ROM 区，其余为 RAM 区。若采用 8K×4 位的 SRAM 芯片进行设计，则需要该芯片的数量是（ ）。
', '[''7'', ''8'', ''14'', ''16'']', "['C']", 'C</font></strong>
参考主存容量的扩展，5FFFH - 4000H + 1 = 2000H，即 ROM 区容量为$2^{13}$B = 8KB（2000H =$2 × 16^{3} = 2^{13}$），RAM 区容量为 56KB（64KB-8KB=56KB），则需要 8 Kx4 位的 SRAM 芯片的数量为 14（56KB/(8Kx4bit)=14）。', 9, 'Mogullzr', '2026-02-02 13:36:41', 9, NULL, '2026-02-02 13:36:41', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/af267cbe056a4dd68f6997b95be7fb8f.svg)

其中`M`为寻址方式，`I`为变址寄存器编号，`D`为形式地址。若采用先变址后间址的寻址方式，则操作数的有效地址是（ ）。
', '[''`I+D`'', ''`(I)+D`'', ''`((I)+D)`'', ''`((I))+D`'']', "['C']", 'C</font></strong>
变址寻址中，有效地址 EA 等于指令字中的形式地址 D 与变址寄存器 I 的内容相加之和，即 EA= (I)+ D。间接寻址是相对于直接寻址而言的，指令的地址字段给出的形式地址不是操作数的真正地址，而是操作数地址的地址，即 EA=(D)。从而该 操作数的有效地址是 ((I)+ D)。', 9, 'Mogullzr', '2026-02-02 13:36:42', 9, NULL, '2026-02-02 13:36:42', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1103', '3645');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3646, '2016年408考研真题第18题', '简单', '2016年408真题', '某计算机主存空间为 4GB，字长为 32 位，按字节编址，采用 32 位定长指令字格式，若指令按字边界对齐存放，则程序计数器 (PC) 和指令寄存器 (IR) 的位数至少分别是（ ）。
', '[''30、30'', ''30、32'', ''32、30'', ''32、32'']', "['B']", 'B</font></strong>
程序计数器（PC）给出下一条指令字的访存地址（指令在内存中的地址），取决于存储器的字数（4 GB / 32bit = 2³⁰），故程序计数器（PC）的位数至少是 30 位；指令寄存器（IR）用于接收取得的指令，取决于指令字长（32 位），故指令寄存器（IR) 的位数至少为 32 位。', 9, 'Mogullzr', '2026-02-02 13:36:42', 9, NULL, '2026-02-02 13:36:42', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3649, '2016年408考研真题第21题', '简单', '2016年408真题', '下列关于总线设计的叙述中的叙述中，错误的是（ ）。
', '[''并行总线传输比串行总线传输速度快'', ''采用信号线复用技术可减少信号线数量'', ''采用突发传输方式可提高总线数据传输率'', ''采用分离事务通信方式可提高总线利用率'']', "['A']", 'A</font></strong>
并行总线传输通常比串行总线传输速度快，但这不是绝对的。在实际时钟频率比较低的情况下，并行总线因为可以同时传输若干比特，速率确实比串行总线快。但是，随着技术的发展，时钟频率越来越高，并行导线之间的相互干扰越来越严重，当时钟频率提高到一定程度时，传输的数据已经无法恢复。而串行总线因为导线少，线间干扰容易控制，反而可以通过不断提高时钟频率来提高传输速率。A错误。总线复用是指一种信号线在不同的时间传输不同的信息。可以使用较少的线路传输更多的信息，从而节省了空间和成本。B正确。突发（猝发）传输是在一个总线周期中，可以传输多个存储地址连续的数据，即一次传输一个地址和一批地址连续的数据。C正确。分离事务通信即总线复用的一种，相比单一的传输线路可以提高总线的利用率。D正确。本题选A。', 9, 'Mogullzr', '2026-02-02 13:36:44', 9, NULL, '2026-02-02 13:36:44', 1, 1);
...
', '[''“访存时缺页” 属于中断'', ''“整数除以 0” 属于异常'', ''“DMA 传送结束” 属于中断'', ''“存储保护错” 属于异常'']', "['A']", 'A</font></strong>
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3651, '2016年408考研真题第23题', '简单', '2016年408真题', '下列关于批处理系统的叙述中，正确的是（  ）
Ⅰ.批处理系统允许多个用户与计算机直接交互
Ⅱ.批处理系统分为单道批处理系统和多道批处理系统
Ⅲ.中断技术使得多道批处理系统和 I/O 设备可与 CPU 并行工作
', '[''仅Ⅱ、Ⅲ'', ''仅Ⅱ'', ''仅Ⅰ、Ⅱ'', ''仅Ⅰ、Ⅲ'']', "['A']", 'A</font></strong>
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/9ad2ee0e0a2a4175aedd35044d214920.svg)
', 9, 'Mogullzr', '2026-02-02 13:36:44', 9, NULL, '2026-02-02 13:36:44', 1, 1);
...
- `(1, 1)`：最近被访问，已被修改 —— 最不愿意淘汰因此，<strong><font color=''#2656b3''>淘汰顺序</font></strong>是按照代价和“是否有用”排序的：👉<strong><font color=''#2656b3''>(0, 0) < (0, 1) < (1, 0) < (1, 1)</font></strong>✅ 正确答案是：<strong><font color=''#2656b3''>A. (0, 0), (0, 1), (1, 0), (1, 1)</font></strong>', 9, 'Mogullzr', '2026-02-02 13:36:45', 9, NULL, '2026-02-02 13:36:45', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/0c1394c9a3324b4eb1d150904c85cb4e.svg)

当访问段号为 2、段内地址为 400 的逻辑地址时，进行地址转换的结果是（ ）。
', '[''段缺失异常'', ''得到内存地址 4400'', ''越权异常'', ''越界异常'']', "['D']", 'D</font></strong>
分段系统的逻辑地址 A 到物理地址 E 之间的地址变换过程参考该节。题目中段号为 2 的段长为 300，小于段内地址为 400，故发生越界异常，D 正确。', 9, 'Mogullzr', '2026-02-02 13:36:47', 9, NULL, '2026-02-02 13:36:47', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/b28edbc13f8f4310aa2412608777eb80.svg)

若工作集的窗口大小为 6，则在 t 时刻的工作集为（ ）。
', '[''{6, 0, 3, 2}'', ''{2, 3, 0, 4}'', ''{0, 4, 3, 2, 9}'', ''{4, 5, 6, 0, 3, 2}'']', "['A']", 'A</font></strong>
在任一时刻 t，都存在一个集合，它包含所有最近 k 次（该题窗口大小为 6）内存访问所访问过的页面。这个集合 w(k, t) 就是工作集。该题中最近 6 次访问的页面分别为 6, 0, 3, 2, 3, 2, 再去除重复的页面，形成的工作集为 {6, 0, 3, 2}。', 9, 'Mogullzr', '2026-02-02 13:36:48', 9, NULL, '2026-02-02 13:36:48', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1213', '3657');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3658, '2016年408考研真题第30题', '简单', '2016年408真题', '进程 P1 和 P2 均包含并发执行的线程，部分伪代码描述如下所示。
进程 P1
```c
int x = 0;
Thread1()
...
下列选项中，需要互斥执行的操作是（ ）。
', '[''a=1 与 a=2'', ''a=x 与 b=x'', ''x+=1 与 x+=2'', ''x+=1 与 x+=3'']', "['C']", 'C</font></strong>
P1 中对 a 进行赋值，并不影响最终的结果，故 a = l 与 a = 2 不需要互斥执行；a = x 与 b = x 执行先后不影响 a 与 b 的结果，无须互斥执行；x+=1 与 x+=2 执行先后会影响 x 的结果，需要互斥执行；P1 中的 x 和 P2 中的 x 是不同范围中的 X，互不影响，不需要互斥执行。', 9, 'Mogullzr', '2026-02-02 13:36:48', 9, NULL, '2026-02-02 13:36:48', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1273', '3658');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3659, '2016年408考研真题第31题', '简单', '2016年408真题', '下列关于 SPOOLing 技术的叙述中，错误的是（ ）。
', '[''需要外存的支持'', ''需要多道程序设计技术的支持'', ''可以让多个作业共享一台独占设备'', ''由用户作业控制设备与输入/输出井之间的数据传送'']', "['D']", 'D</font></strong>
SPOOLing是利用专门的外围控制机，将低速 I/O 设备上的数据传送到高速磁盘上，或者相反。SPOOLing 的意思是外部设备同时联机操作，又称为假脱机输入／输出操作，是操作系统中采用的一项将独占设备改造成共享设备的技术。高速磁盘即外存，A 正确。SPOOLing 技术需要进行输入/输出操作，单道批处理系统无法满足，B 正确。SPOOLing 技术实现了将独占设备改造成共享设备的技术，C 正确。设备与输入／输出井之间数据的传送是由系统实现的，D 错误。', 9, 'Mogullzr', '2026-02-02 13:36:48', 9, NULL, '2026-02-02 13:36:48', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1274', '3659');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3660, '2016年408考研真题第32题', '简单', '2016年408真题', '下列关于管程的叙述中，错误的是（ ）。
', '[''管程只能用于实现进程的互斥'', ''管程是由编程语言支持的进程同步机制'', ''任何时候只能有一个进程在管程中执行'', ''管程中定义的变量只能被管程内的过程访问'']', "['A']", 'A</font></strong>
管程是由一组数据以及定义在这组数据之上的对这组数据的操作组成的软件模块，这组操作能初始化并改变管程中的数据和同步进程。管程不仅能实现进程间的互斥，而且能实现进程间的同步，故 A 错误、B 正确。管程具有特性：①局部于管程的数据只能被局部于管程内的过程所访问；②一个进程只有通过调用管程内的过程才能进入管程访问共享数据；③每次仅允许一个进程在管程内执行某个内部过程，故 C 和 D 正确。', 9, 'Mogullzr', '2026-02-02 13:36:49', 9, NULL, '2026-02-02 13:36:49', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1275', '3660');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3661, '2016年408考研真题第33题', '简单', '2016年408真题', '在 OSI 参考模型中，R1、Switch、Hub 实现的最高功能层分别是（ ）。
', '[''2、2、1'', ''2、2、2'', ''3、2、1'', ''3、2、2'']', "['C']", 'C</font></strong>
本题考察计算机网络设备，集线器是一个多端口的中继器，工作在物理层。以太网交换机是一个多端口的网桥，工作在数据链路层。路由器是网络层设备，它实现了网络模型的下三层，即物理层、数据链路层和网络层。题中 R1、Switch 和 Hub 分别是路由器、交换机和集线器，实现的最高层功能分别是网络层（即 3）、数据链路层（即 2）和物理层（即 1）。', 9, 'Mogullzr', '2026-02-02 13:36:49', 9, NULL, '2026-02-02 13:36:49', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3662, '2016年408考研真题第34题', '简单', '2016年408真题', '若连接 R2 和 R3 链路的频率带宽为 8 kHz，信噪比为 30 dB，该链路实际数据传输速率约为理论最大数据传输速率的 50％，则该链路的实际数据传输速率约是（ ）。
', '[''8 kbps'', ''20 kbps'', ''40 kbps'', ''80 kbps'']', "['C']", 'C</font></strong>
...
$$
所以', 9, 'Mogullzr', '2026-02-02 13:36:49', 9, NULL, '2026-02-02 13:36:49', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1239', '3662');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3663, '2016年408考研真题第35题', '简单', '2016年408真题', '若主机 H2 向主机 H4 发送 1 个数据帧，主机 H4 向主机 H2 立即发送一个确认帧，则除 H4 外，从物理层上能够收到该确认帧的主机还有（ ）。
', '[''仅 H2'', ''仅 H3'', ''仅 H1、H2'', ''仅 H2、H3'']', "['D']", 'D</font></strong>
参考计算机网络设备，交换机可以隔离冲突域，但集线器无法隔离冲突域，因此从物理层上能够收到该确认帧的主机仅 H2、H3, 选项 D 正确。', 9, 'Mogullzr', '2026-02-02 13:36:49', 9, NULL, '2026-02-02 13:36:49', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1163', '3663');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3664, '2016年408考研真题第36题', '简单', '2016年408真题', '若 Hub 再生比特流过程中，会产生 1.535μs 延时，信号传播速度为 200m/μs，不考虑以太网帧的前导码，则 H3 与 H4 之间理论上可以相距的最远距离是（ ）。
', '[''200 m'', ''205 m'', ''359 m'', ''512 m'']', "['B']", 'B</font></strong>
因为要解决理论上可以相距的最远距离，所以最远肯定要保证能检测到碰撞，而以太网规定最短帧长为 64B，其中 Hub 为 100Base-T 集线器，可知线路的传输速率为 100Mbps, 则单程传输时延为 64B/100Mbps/2 = 2.56μs，又 Hub 再产生比特流的过程中会导致延时 1.535μs，则单程的传播时延为 2.56μs- 1.535μs = 1.025μs，从而 H3 与 H4 之间理论上可以相距的最远距离为 200m/μs x 1.025μs = 205m。', 9, 'Mogullzr', '2026-02-02 13:36:49', 9, NULL, '2026-02-02 13:36:49', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1172', '3664');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3665, '2016年408考研真题第37题', '简单', '2016年408真题', '假设 R1、R2、R3 采用 RIP 协议交换路由信息，且均已收敛。若 R3 检测到网络 201.1.2.0/25 不可达，并向 R2 通告一次新的距离向量，则 R2 更新后，其到达该网络的距离是（ ）。
', '[''2'', ''3'', ''16'', ''17'']', "['B']", 'B</font></strong>
因为 R3 检测到网络 201.1.2.0/25 不可达，故将到该网络的距离设置为 16（距离为 16 表示不可达）。当 R2 从 R3 收到路由信息时，因为 R3 到该网络的距离为 16，则 R2 到该网络也不可达，但此时记录 R1 可达（由千 RIP 的特点坏消息传得慢，R1 并没有收到 R3 发来的路由信息），R1 到该网络的距离为 2，再加上从 R2 到 Rl 的 l 就是 R2 到该网络的距离 3。', 9, 'Mogullzr', '2026-02-02 13:36:49', 9, NULL, '2026-02-02 13:36:49', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1276', '3665');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3666, '2016年408考研真题第38题', '简单', '2016年408真题', '假设连接 R1、R2 和 R3 之间的点对点链路使用 201.1.3.x/30 地址，当 H3 访问 Web 服务器 S 时，R2 转发出去的封装 HTTP 请求报文的 IP 分组的源 IP 地址和目的 IP 地址分别是（ ）。
', '[''192.168.3.251，130.18.10.1'', ''192.168.3.251，201.1.3.9'', ''201.1.3.8，130.18.10.1'', ''201.1.3.10，130.18.10.1'']', "['D']", 'D</font></strong>
由题意知连接 R1、R2 和 R3 之间的点对点链路使用 201.1.3.x/30 地址，其子网掩码为 255.255.255.252，R1 的一 个接口的 IP 地址为 201.1.3.9，转换为对应的二进制的后 8 位为 0000 1001（由 201.1.3.x/30 知，IP 地址对应的二进制的后两位为主机号，而主机号全为 0 表示本网络本身，主机号全为 1 表示本网络的广播地址，不用于源 IP 地址或者目的 IP 地址），那么除 201.1.3.9 外，只有 IP 地址为 201.1.3.10 才可以作为源 IP 地址使用（本题为 201.1.3.10)。Web 服务器的 IP 地址为 130.18.10.1，作为 IP 分组的目的 IP 地址。综上可知，选项 D 正确。', 9, 'Mogullzr', '2026-02-02 13:36:49', 9, NULL, '2026-02-02 13:36:49', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1211', '3666');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3667, '2016年408考研真题第39题', '简单', '2016年408真题', '假设 H1 与 H2 的默认网关和子网掩码均分别配置为 192.168.3.1 和 255.255.255.128，H3 与 H4 的默认网关和子网掩码均分别配置为 192.168.3.254 和 255.255.255.128，则下列现象中可能发生的是（ ）。
', '[''H1 不能与 H2 进行正常 IP 通信'', ''H2 与 H4 均不能访问 Internet'', ''H1 不能与 H3 进行正常 IP 通信'', ''H3 不能与 H4 进行正常 IP 通信'']', "['C']", 'C</font></strong>
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3668, '2016年408考研真题第40题', '简单', '2016年408真题', '假设所有域名服务器均采用迭代查询方式进行域名解析。当 H4 访问规范域名为 www.abc.xyz.com 的网站时，域名服务器 201.1.1.1 在完成该域名解析过程中，可能发出 DNS 查询的最少和最多次数分别是（ ）。
', '[''0，3'', ''1，3'', ''0，4'', ''1，4'']', "['C']", 'C</font></strong>
最少情况下：当本机DNS 缓存中存有该域名的 DNS 信息时，则不 需要查询任何域名服务器，这样最少发出 0 次 DNS 查询。最多情况下：因为均采用迭代查询的方式，在最坏的情况下，需要依次迭代地向本地域名服务器、根域名服务器 (.com)、顶级域名服务器 (xyz.com)、权限域名服务器 (abc.xyz.com) 发出 DNS 查询 请求，因此 最多发出 4 次 DNS 查询。', 9, 'Mogullzr', '2026-02-02 13:36:49', 9, NULL, '2026-02-02 13:36:49', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/5845db2e8d184cd19642f72ae9ed9ec7.svg)
...
', 9, 'Mogullzr', '2026-02-02 13:36:51', 9, NULL, '2026-02-02 13:36:51', 1, 0);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3670, '2016年408考研真题第42题', '简单', '2016年408真题', '如果一棵非空$k$（$k ≥ 2$） 叉树$T$中每个非叶结点都有$k$个孩子，则称$T$为正则$k$叉树。请回答下列问题并给出推导过程。
(1) 若$T$有$m$个非叶结点，则$T$中的叶结点有多少个？
(2) 若$T$的高度为$h$（单结点的树$h = 1$），则$T$的结点数最多为多少个？最少为多少个？
', '[]', '', '1）根据定义，正则$k$叉树中仅含有两类结点；叶结点（个数记为$n_{0}$）和度为$k$的分支结点（个数记为$n_{1}$）。树$T$中的结点总数$n = n_{0} + n_{k} = n_{0} + m$。树中所含的边数$e = n - 1$，这些边均为$m$个度为$k$的结点发出的，即$e = mk$。整理得$n_{0} + m = m_{k} + 1$，故$n_{0} = (k - 1)m + 1$。
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3672, '2016年408考研真题第44题', '简单', '2016年408真题', '假定 CPU 主频为 50MHz，CPI 为 4。设备 D 采用异步串行通信方式向主机传送 7 位 ASCII 字符，通信规程中有 1 位奇校验位和 1 位停止位，从 D 接收启动命令到字符送入 I/O 端口需要 0.5ms。请回答下列问题，要求说明理由。
(1) 每传送一个字符，在异步串行通信线上共需传输多少位？在设备 D 持续工作过程中，每秒钟最多可向 I/O 端口送入多少个字符？
(2) 设备 D 采用中断方式进行输入/输出，示意图如下∶

![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/9637392ee3df409caa32d16362818bae.svg)
...
', 9, 'Mogullzr', '2026-02-02 13:36:53', 9, NULL, '2026-02-02 13:36:53', 1, 0);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/02/d293adbcb23a4d5d9235584453250f33.svg)
...
', 9, 'Mogullzr', '2026-02-02 13:36:54', 9, NULL, '2026-02-02 13:36:54', 1, 0);
...
', 9, 'Mogullzr', '2026-02-02 13:36:56', 9, NULL, '2026-02-02 13:36:56', 1, 0);
