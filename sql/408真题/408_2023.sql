INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3958, '2023年408考研真题第1题', '简单', '2023年408真题', '下列对顺序存储的有序表（长度为 n）实现给定操作的算法中平均时间复杂度为 O(1) 的是（ ）。', '[''查找包含指定值元素的算法'', ''插入包含指定值元素的算法'', ''删除第 i 个元素的算法'', ''获取第 i 个值的算法'']', "['D']", 'D
线性表的顺序存储结构采用一组地址连续的存储单元依次存储线性表的数据元素。特点是逻辑上相邻的数据元素在物理位置上相邻。线性表顺序存储结构是一种随机存取的存储结构，设线性表的每个元素占 L 个存储单元，第一个元素$a_{1}$的存储地址是 LOC($a_{1}$)，则任意元素$a_{i}$的 LOC(ai)=LOC($a_{1}$)+(i-1)*L。因此获取第 i 个值的算法为常量阶 O(1)。', 9, 'Mogullzr', '2026-01-29 18:56:12', 9, NULL, '2026-01-29 18:56:12', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/5df5d4f2399942c5b2ec8bda1a5fded3.svg)
prev 是指向前直接前驱结点的指针，next 是指向直接后继结点的指针。若要在 L 中指针 p 所指向的结点（非尾结点）之后插入指针 s 指向的新结点，则在执行了语句序列：s->next=p->next; p->next=s，后，还要执行（ ）。', '[''s->next->prev=p; s->prev=p;'', ''p->next->prev=s; s->prev=p;'', ''s->prev=s->next->prev; s->next->prev=s;'', ''p->next->prev=s->prev; s->next->prev=p;'']', "['C']", 'C
主要考察双链表的插入操作，解决这类问题可在纸上画出具体的双链表进行模拟。因为s->next已经赋值为p的后一个结点，同时p->next指针已经赋值为s。所以只需要处理s->next->prev和s->next->prev的赋值，s->prev需要指向p，s-next->prev需要指向s。因为p->next和s指向同一个结点，所以可以用p->next代表s。故本题的正确选项为 C。', 9, 'Mogullzr', '2026-01-29 18:56:13', 9, NULL, '2026-01-29 18:56:13', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1130', '3959');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3960, '2023年408考研真题第3题', '简单', '2023年408真题', '若采用三元组表存储结构存储系数矩阵 M。则除三元组外，下列数据中还需要保存的是（ ）。I. M 的行数II. M 中包含非零元素的行数III. M 的列数IV. M 中包含非零元素的列数', '[''仅 I 和 III'', ''仅 I 和 IV'', ''仅 II 和 IV'', ''I, II, III, IV'']', "['A']", 'A
存储稀疏矩阵 M，三元组表的表项存储了行 row、列 col、值 value 三种信息，除此之外，我们还需要知道矩阵 M 的规模 rows × cols，即 M 的行数 rows 和 M 的列数 cols，这个信息应该直接给出。当我们需要某个位置的元素，可以先根据 M 的行数和列数判断是否越界，如果没有越界，在三元组进行查找，如果三元组没有保存对应位置的值代表矩阵中该位置的值为 0。本题答案选 A。', 9, 'Mogullzr', '2026-01-29 18:56:14', 9, NULL, '2026-01-29 18:56:14', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3961, '2023年408考研真题第4题', '简单', '2023年408真题', '在有 6 个字符组成的字符集 S 中，各个字符出现的频次分别为 3, 4, 5, 6, 8, 10，为 S 构造的哈夫曼树的加权平均长度为（ ）', '[''2.4'', ''2.5'', ''2.67'', ''2.75'']', "['B']", 'B
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/b19b9f403ade4ec18008ff2f34b262ca.svg)
', 9, 'Mogullzr', '2026-01-29 18:56:14', 9, NULL, '2026-01-29 18:56:14', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/eb739b2b6b6e435791ce51559bad75c4.svg)
', '[''aedfbc'', ''acebdf'', ''cabefd'', ''dfebac'']', "['A']", 'A
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/548463b1c507490088729c1e49ad18bb.svg)
', 9, 'Mogullzr', '2026-01-29 18:56:16', 9, NULL, '2026-01-29 18:56:16', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1243', '3962');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3963, '2023年408考研真题第6题', '简单', '2023年408真题', '已知无向连通图 G 中各边的权值均为 1，下列算法中，一定能够求出图 G 中从某顶点到其余各个顶点最短路径的是（ ）。I. 普利姆算法II. 克鲁斯卡尔算法III. 图的广度优先搜索', '[''仅 I'', ''仅 III'', ''仅 II 和 I'', ''I,II,III'']', "['B']", 'B
普里姆算法和克鲁斯卡尔算法是用于解决最小生成树问题的算法，它们并不直接适用于求解最短路径问题。广度优先遍历 (BFS) 是一种图遍历算法，可以用于求解单源最短路径问题。在权值均为 1 的情况下，BFS 算法从给定的起始顶点开始遍历，按照层级顺序逐步扩展，每次扩展一层。这样，当 BFS 遍历到目标顶点时，它所经过的边数就是从起始顶点到目标顶点的最短路径长度。因此，BFS 算法可以用于求解图 G 中从某个顶点到其余各个顶点的最短路径。本题答案选 B。', 9, 'Mogullzr', '2026-01-29 18:56:18', 9, NULL, '2026-01-29 18:56:18', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1200', '3963');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3964, '2023年408考研真题第7题', '简单', '2023年408真题', '下列关于非空 B 树的叙述中，正确的是（ ）I. 插入操作可能增加树的高度II. 删除操作一定会导致叶结点的变化III. 查找某关键字一定是要查找到叶结点IV. 插入的新关键字最终位于叶结点中', '[''仅 I'', ''仅 I、II'', ''仅 III、IV'', ''仅 I、II、IV'']', "['B']", 'B
在B 树中，插入操作可能导致节点分裂。如果根节点已满，插入新键会触发根节点分裂，生成一个新的根节点，从而增加树的高度。因此，插入操作确实可能（但不一定）增加树的高度。选项 I 正确。删除操作可能涉及以下情况：删除的键在叶子结点：直接从叶子结点移除该键，导致叶子结点内容变化。删除的键在非叶子结点：B 树会用前驱或后继键（通常位于叶子结点）替换该键，然后删除前驱或后继键。这仍然会导致叶子结点的变化。删除后触发合并或借键：如果删除键后某个节点键数低于最小要求（通常 ⌈m / 2⌉ − 1），可能需要从兄弟节点借键或与兄弟节点合并。这些操作可能涉及叶子结点（例如，合并两个叶子结点或调整叶子结点的键）。在所有删除场景中，无论是直接删除键还是通过替换、合并、借键，操作最终都会影响叶子结点（键的移除或调整发生在叶子结点）。选项 II 正确。查找关键字并不一定需要查找到叶子结点。例如，如果目标键位于根节点或中间节点，查找会在这些非叶子结点停止。选项 III 错误。新关键字在 B 树中开始总是被插入到叶子结点中，但是可能随着分裂上溢到非叶子结点，选项 IV 错误。', 9, 'Mogullzr', '2026-01-29 18:56:18', 9, NULL, '2026-01-29 18:56:18', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1094', '3964');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3965, '2023年408考研真题第8题', '简单', '2023年408真题', '对含有 600 个元素的有序顺序表进行折半查找，关键字之间的比较次数最多是（ ）。', '[''9'', ''10'', ''30'', ''300'']', "['B']", 'B
...
位置上的关键字已被删除。如下表所示，查找失败的平均查找长度为 (1+3+2+1+2)/5=1.8。本题答案选 C。| 地址     | 0 | 1    | 2  | 3 | 4           |
|:------:|:-:|:----:|:--:|:-:|:-----------:|
| Key    |   | 2022 | 12 |   | 25 (delete) |
| 查找失败次数 | 1 | 3    | 2  | 1 | 2           |
', 9, 'Mogullzr', '2026-01-29 18:56:18', 9, NULL, '2026-01-29 18:56:18', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3967, '2023年408考研真题第10题', '简单', '2023年408真题', '下列排序算法中，不稳定的是（ ）I. 希尔排序II. 归并排序III. 快速排序IV. 堆排序V. 基数排序', '[''仅 I 和 II'', ''仅 II 和 V'', ''仅 I，III，IV'', ''仅 III，IV，V'']', "['C']", 'C
...
本题的正确答案为 C。', 9, 'Mogullzr', '2026-01-29 18:56:18', 9, NULL, '2026-01-29 18:56:18', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1131', '3969');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3970, '2023年408考研真题第13题', '简单', '2023年408真题', '若 short 型变量 x = -8190，则 x 的机器数为（ ）', '[''E002H'', ''E001H'', ''9FFFH'', ''9FFEH'']', "['A']", 'A
...
正确选项为 A。', 9, 'Mogullzr', '2026-01-29 18:56:18', 9, NULL, '2026-01-29 18:56:18', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1099', '3970');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3971, '2023年408考研真题第14题', '简单', '2023年408真题', '已知 float 型变量用 IEEE754 单精度浮点数格式表示。若 float 型变量 x 的机器数为 8020 0000H，则 x 的值（ ）', '[''$−2^{−128}$'', ''$−1.01 × 2^{−127}$'', ''$−1.01 × 2^{−126}$'', ''非数 (NAN)'']', "['A']", 'A
...
不是 1.M，同时指数部分是 -126，而不是 -127，则真值为$−0.01 × 2^{−126} = −2^{−128}$，正确答案为 A 选项。', 9, 'Mogullzr', '2026-01-29 18:56:18', 9, NULL, '2026-01-29 18:56:18', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1176', '3971');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3972, '2023年408考研真题第15题', '简单', '2023年408真题', '某计算机的 CPU 有 30 根地址线，按字节编址，CPU 和主存芯片连接时，要求主存芯片占满所有可能存储地址空间，并且 RAM 区和 ROM 区所分配的容量大小比为 3:1，若 RAM 在连续低地址区，ROM 在连续高地址区，则 ROM 的地址范围（ ）', '[''00000000H ~ 0FFFFFFFH'', ''10000000H ~ 2FFFFFFFH'', ''30000000H ~ 3FFFFFFFH'', ''40000000H ~ 4FFFFFFFH'']', "['C']", 'C
...
（30 个 1)，十六进制表示为 300000000H ~ 3FFFFFFFFH。故本题的正确选项为 C。', 9, 'Mogullzr', '2026-01-29 18:56:18', 9, NULL, '2026-01-29 18:56:18', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1322', '3972');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3973, '2023年408考研真题第16题', '简单', '2023年408真题', '已知 x、y 为 int 类型，当 x=100，y=200 时，执行 x-y 指令的到的溢出标志 OF 和借位标志 CF 分别为 0,1，那么当 x=10，y=-20 时，执行该指令得到的 OF 和 CF 分别是（ ）', '[''0, 0'', ''0, 1'', ''1, 0'', ''1, 1'']', "['B']", 'B
int 类型在计算机中以补码形式表示，因此，x（10）的二进制表示为 0000 0000 0000 0000 0000 0000 0000 1010B，y（-20）的二进制表示为 11111111 11111111 1111 1111 11101100。OF 和 CF 是 CPU 中标志寄存器的标志位。CF 表示无符号整数运算时的进位/借位，因此计算 CF 的时候，需要把和 x 和 y 当成无符号数进行计算，因为 x 不够减 y，所以 CF = 1。计算 OF 的时候，需要把和 x 和 y 当成有符号数进行计算，x-y=30，不超过 int 的最大值，不会产生溢出，所以 OF=0。所以本题的正确选项为 B。', 9, 'Mogullzr', '2026-01-29 18:56:19', 9, NULL, '2026-01-29 18:56:19', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1177', '3973');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3974, '2023年408考研真题第17题', '简单', '2023年408真题', '某运算类型指令中有一个地址码为通用寄存器编号，对应通用寄存器中存放的是操作数或操作数地址，CPU 区分两者的依据是（ ）', '[''操作数的寻址方式'', ''操作数的编码方式'', ''通用寄存器编号'', ''通用寄存器的内容'']', "['A']", 'A
在指令中，使用不同的操作码来区分寄存器中存放的是操作数还是操作数的地址指令会有明确的寻址方式，指示使用寄存器中的内容作为操作数或操作数地址。当 CPU 在执行指令时会根据指令中的寻址方式字段来正确解析寄存器中的内容，并根据需要进行相应的操作。所以本题的正确选项为 A。', 9, 'Mogullzr', '2026-01-29 18:56:19', 9, NULL, '2026-01-29 18:56:19', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1103', '3974');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3975, '2023年408考研真题第18题', '简单', '2023年408真题', '数据通路由组合逻辑元件（操作元件）和时序逻辑元件（状态元件）组成。下列给出的元件中，属于操作元件的是（ ）。I. 算术逻辑部件（ALU）II. 程序计数器（PC）III. 通用寄存器组（GPRs）IV. 多路选择题（MUX）', '[''仅 I、II'', ''仅 I、IV'', ''仅 II、III'', ''仅 I、II、IV'']', "['B']", 'B
组合逻辑元件是那些仅由组合逻辑电路组成的元件，其输出仅取决于当前的输入，而不受存储器或时钟信号的影响。算术逻辑部件 ALU 和多路选择器 MUX 都属于组合逻辑元件，它们的输出仅由当前输入决定。程序计数器 PC 和通用寄存器是时序逻辑元件，也称为状态元件。它们的输出不仅取决于当前输入，还受存储器或时钟信号的影响，并且具有状态或存储功能。因此本题选 B。', 9, 'Mogullzr', '2026-01-29 18:56:19', 9, NULL, '2026-01-29 18:56:19', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1227', '3975');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3976, '2023年408考研真题第19题', '简单', '2023年408真题', '在采用“取指、译码/取数、执行、访存、写回”5 段流水线的 RISC 处理器中，执行如下指令序列（第一列为指令序号），其中 s0、s1、s2、s3 和 t2 表示寄存器编号。若采用转发（旁路）技术处理数据冒险，采用硬件阻塞方式处理控制冒险，则在 I1~I4 执行过程中，发生流水线阻塞的指令有（ ）。', '[''仅 I3'', ''仅 I2、I4'', ''仅 I3、I4'', ''仅 I2、I3、I4'']', "['C']", 'C
在以下指令序列中：I1：写寄存器R[s2]I2：使用R[s2]计算访存地址I3：使用R[s3]进行比较跳转I4：紧跟在beq指令之后执行I2 与 I1 之间：写后读数据冒险（RAW Hazard）指令 I1 将结果写入寄存器R[s2]，但这个值直到 I1 的写回（WB）阶段才会更新。而 I2 在其执行（EX）阶段就需要用到R[s2]来计算访存地址，因此存在写后读的数据冒险。幸运的是，I1 在EX阶段末尾就已生成结果并存储在 EX→M 流水段寄存器中。此时如果使用旁路转发，可以直接将这个值从流水段寄存器中转发到 I2 的 ALU 输入端，避免等待 WB 阶段，从而解决了这个 RAW 冒险。I3 与 I2 之间：装入-使用数据冒险（Load-Use Hazard）I2 是一条load指令，它需要从内存中将数据装入寄存器R[s3]。而 I3 在它的 EX 阶段就要用到R[s3]的值。如果像前面那样尝试用旁路技术从 I2 的 EX→M 寄存器转发数据，会失败——因为此时的数据尚未从内存中读出。只有等到 I2 的访存（M）阶段结束，数据才真正从内存中取回，存放在 M→WB 流水段寄存器。因此，仅靠转发线路无法解决装入-使用冒险。需要采取的办法是：对 I3硬件阻塞一个周期，等待 I2 的 M 阶段完成，然后通过转发将R[s3]的新值从 M→WB 段寄存器送入 I3 的 ALU，以确保其操作正确。I4 与 I3 之间：控制冒险（Control Hazard）I3 是一条beq分支指令。该指令在 EX 阶段比较寄存器值，在M 阶段才判断跳转是否成立并更新 PC（程序计数器）。因此，在得知是否跳转前，I4 的取指（IF）阶段必须等待 I3 的 M 阶段完成。这就导致 I4 的执行被阻塞 3 个时钟周期，以确保其是否被执行是基于正确的控制流判断。最终，系统采用硬件阻塞机制来处理上述数据和控制冒险，下图展示了指令执行过程中各阶段的具体安排：
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/86248bd2a3c44e27a1f1e94c39612841.jpg)
', 9, 'Mogullzr', '2026-01-29 18:56:19', 9, NULL, '2026-01-29 18:56:19', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1151', '3976');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3977, '2023年408考研真题第20题', '简单', '2023年408真题', '某存储器总线宽度为 64 位，总线时钟频率为 1GHZ，在总线上传输一个数据或地址需要一个的时钟周期，不支持突发传送方式，若通过该总线连接 CPU 和主存，主存每次准备一个 64 位数据需要 6ns，主存块大小为 32B，则读取一个主存块需要的时间为（ ）。', '[''8ns'', ''11ns'', ''26ns'', ''32ns'']', "['D']", 'D
...
主存块的时间为 4×8ns=32ns。所以本题的正确选项为 D 选项。', 9, 'Mogullzr', '2026-01-29 18:56:21', 9, NULL, '2026-01-29 18:56:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1107', '3977');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3978, '2023年408考研真题第21题', '简单', '2023年408真题', '下列关于硬件和异常/中断关系的叙述中，错误的是（ ）。', '[''CPU 在执行一条指令过程中检测异常事件'', ''CPU 在执行完一条指令时检测中断请求信号'', ''开中断中 CPU 检测到中断请求后就进行中断响应'', ''外部设备通过中断控制器向 CPU 发中断结束信号'']', "['D']", 'D
...
号，选项 D 错误。', 9, 'Mogullzr', '2026-01-29 18:56:21', 9, NULL, '2026-01-29 18:56:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1109', '3978');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3979, '2023年408考研真题第22题', '简单', '2023年408真题', '下列关于 I/O 控制方式的叙述中，错误的是（ ）。', '[''查询方式下，通过 CPU 执行查询程序进行 I/O 操作'', ''中断方式下，通过 CPU 执行中断服务程序进行 I/O 操作'', ''DMA 方式下，通过 CPU 执行 DMA 传送程序进行 I/O 操作'', ''对于 SSD 网络适配器等高速设备，采用 DMA 方式输入/输出'']', "['C']", 'C
...
择错误的选项，因此本题的答案为 C。', 9, 'Mogullzr', '2026-01-29 18:56:21', 9, NULL, '2026-01-29 18:56:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1294', '3979');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3980, '2023年408考研真题第23题', '简单', '2023年408真题', '与宏内核操作系统相比，下列特征中微内核操作系统具有的是（ ）。Ⅰ. 较好的性能Ⅱ. 较高的可靠性Ⅲ. 较高的安全性Ⅳ. 较强的可扩展性', '[''Ⅱ、Ⅳ'', ''Ⅰ、Ⅱ、Ⅲ'', ''Ⅰ、Ⅲ、Ⅳ'', ''Ⅱ、Ⅲ、Ⅳ'']', "['D']", 'D
参考微内核和宏内核，
...
系统所具有的特点，正确的选项为 II，III，IV，选择 D 选项。', 9, 'Mogullzr', '2026-01-29 18:56:21', 9, NULL, '2026-01-29 18:56:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1110', '3980');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3981, '2023年408考研真题第24题', '简单', '2023年408真题', '在操作系统内核中，中断向量表适合采用的数据结构是（ ）。', '[''数组'', ''队列'', ''单向链表'', ''双向链表'']', "['A']", 'A
...
题的正确选项为 A。', 9, 'Mogullzr', '2026-01-29 18:56:21', 9, NULL, '2026-01-29 18:56:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1132', '3981');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3982, '2023年408考研真题第25题', '简单', '2023年408真题', '某系统采用页式存储管理，用位图管理空闲页框。若页大小为 4 KB，物理内存大小为 16 GB，则位图所占空间的大小是（ ）。', '[''128 B'', ''128 KB'', ''512 KB'', ''4 MB'']', "['C']", 'C
物理内存大小为$16GB$，每页大小为$4KB$，因此物理内存总共包含的页框数为$(16GB)/(4KB) = 2^{34}/2^{12} = 2^{22}$。位图需要管理每个页框的空闲状态，因此位图的位数应该等于物理
内存中页框的数量。每个位表示一个页框的空闲状态，所以位图所占空间大小应为$（位图所占位数）/8 = (2^{22})/8 = 2^{19}bytes = 512KB$，因此，位图所占空间的大小为$512KB$，选项$C$为正确答案。', 9, 'Mogullzr', '2026-01-29 18:56:21', 9, NULL, '2026-01-29 18:56:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1236', '3982');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3983, '2023年408考研真题第26题', '简单', '2023年408真题', '下列操作完成时，导致 CPU 从内核态转为用户态的是（ ）。', '[''阻塞过程'', ''执行 CPU 调度'', ''唤醒进程'', ''执行系统调用'']', "['D']", 'D
来看每个选项的含义和状态切换：对于 A 选项，阻塞过程属于进程状态切换，不直接涉及CPU模式转换。对于 B 选项，执行 CPU 调度属于内核调度器行为，通常保持在内核态。对于 C 选项，唤醒进程属于进程管理操作，不直接触发模式切换对于 D 选项，当用户程序需要访问特权指令或执行需要操作系统提供的服务时，它会通过系统调用请求进人内核态。在系统调用期间，CPU 会切换到内核态执行相应的内核代码来满足用户程序的需求。一旦系统调用执行完毕，CPU 会从内核态返回到用户态继续执行用户程序。所以，执行系统调用是导致 CPU 从内核态转为用户态的关键操作。所以该题的正确选项为 D 选项。', 9, 'Mogullzr', '2026-01-29 18:56:21', 9, NULL, '2026-01-29 18:56:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1182', '3983');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3984, '2023年408考研真题第27题', '简单', '2023年408真题', '下列出当前线程引起的事件或执行的操作中，可能导致该线程由执行态变为就绪态的是（ ）。', '[''键盘输入'', ''缺页异常'', ''主动出让 CPU'', ''执行信号量的 wait() 操作'']', "['C']", 'C
参考状态转化，
...
所以本题的答案为 D 选项。', 9, 'Mogullzr', '2026-01-29 18:56:21', 9, NULL, '2026-01-29 18:56:21', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1138', '3985');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3986, '2023年408考研真题第29题', '简单', '2023年408真题', '进程 P1、P2 和 P3 进入就绪队列的的时刻，优先值（越大优先权越高）以及 CPU 的执行时间如下表所示。系统采用基于优先权的抢占式 CPU 调度算法，从 0ms 时刻开始进行调度，则 P1、P2 和 P3 的平均周转时间为（ ）。', '[''60 ms'', ''61 ms'', ''70 ms'', ''71 ms'']', "['B']", 'B
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/276aaa9ee13c4a98b3f4b88e89cb08bd.svg)
', 9, 'Mogullzr', '2026-01-29 18:56:22', 9, NULL, '2026-01-29 18:56:22', 1, 1);
...
同的物理页框中，不同进程的虚拟地址映射到相同的物理地址。所以本题的正确选项为 C 选项。', 9, 'Mogullzr', '2026-01-29 18:56:23', 9, NULL, '2026-01-29 18:56:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1138', '3987');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3988, '2023年408考研真题第31题', '简单', '2023年408真题', '若文件 F 仅被进程 P 打开并访问，则当进程 P 关闭 F 时，下列操作中，文件系统需要完成的是（ ）。', '[''删除目录中文件 F 的目录项'', ''释放 F 的索引节点所占的内存空间'', ''释放 F 的索引节点所占的外存空间'', ''将文件磁盘索引节点中的链接计数减 1'']', "['B']", 'B
...
时，需释放 F 的索引节点所占的内存空间。所以该题的正确答案为 B 选项。', 9, 'Mogullzr', '2026-01-29 18:56:23', 9, NULL, '2026-01-29 18:56:23', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1117', '3988');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3989, '2023年408考研真题第32题', '简单', '2023年408真题', '下列因素中，设备分配需要考虑的是（ ）。Ⅰ. 设备的类型Ⅱ. 设备的访问权限Ⅲ. 设备的占用状态Ⅳ. 逻辑设备与物理设备的映射关系', '[''Ⅰ、Ⅱ'', ''Ⅱ、Ⅲ'', ''Ⅲ、Ⅳ'', ''Ⅰ、Ⅱ、Ⅲ、Ⅳ'']', "['D']", 'D
...
选项是 I、II、III、IV。本题的正确选项为 D。', 9, 'Mogullzr', '2026-01-29 18:56:23', 9, NULL, '2026-01-29 18:56:23', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/945f650891c54777b94932c590c3a046.svg)
', '[''80.02ms'', ''80.08ms'', ''80.09ms'', ''80.10ms'']', "['D']", 'D
| 参数        | 计算                      | 结果            |
|:---------:|:-----------------------:|:-------------:|
| 链路带宽R     | —                       | 100Mbps       |
| 时延带宽积RTp​ | 题设给定                    | 1000bit       |
| 传播时延Tp​   | Tp​=100Mbps1000bit​     | 0.01 ms       |
| 分组大小L     | —                       | 1000B=8000bit |
| 发送时延Ts​   | Ts​=RL​=100Mbps8000bit​ | 0.08 ms       |
| 分组数量N     | N=1000B1MB​             | 1000          |
发送阶段第 1 个分组：$0 → 0.08$ms 发送完。…第 1000 个分组：$79.92 → 80$ms 发送完。因此，最后 1 位比特离开 H1 的时刻为$t_{1} = 80 ms$。H1 → R 传播单向传播时延$T_{p} = 0.01$ms
$t_{2} = t_{1} + T_{p} = 80.01 ms$R 转发（序列化）路由器必须将该分组重新发送一次，耗时仍为$T_{s} = 0.08$ms
$t_{3} = t_{2} + T_{s} = 80.09 ms$R → H2 传播再次经过链路传播时延$T_{p} = 0.01$ms
$t_{4} = t_{3} + T_{p} = 80.10 ms$所以答案选择 D。', 9, 'Mogullzr', '2026-01-29 18:56:24', 9, NULL, '2026-01-29 18:56:24', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1240', '3990');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3991, '2023年408考研真题第34题', '简单', '2023年408真题', '某无噪声理想信道带宽为 4MHz，采用 QAM 调制，若该信道的最大数据传输率是 48Mbps，则该信道采用的 QAM 调制方案是（）', '[''QAM-16'', ''QAM-32'', ''QAM-64'', ''QAM-128'']', "['C']", 'C
对于无噪声理想信道，信道的最大数据传输率可以通过奈奎斯特定理计算，最大数据传输的次数为 2×4MHz=8MHz，代表每秒传输 8M 次，因为最大的数据传输率为 48Mbps，所以每次需要传输的比特数量为 48Mbps/8MHz=6 位，所以每次传输的数据量位 6 比特，那么采用的 QAM 调制方案为 26=64，即采用 QAM-64 的方案。本题的正确选项为 C。', 9, 'Mogullzr', '2026-01-29 18:56:24', 9, NULL, '2026-01-29 18:56:24', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1286', '3991');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3992, '2023年408考研真题第35题', '简单', '2023年408真题', '假设通过同一信道，数据链路层分别采用停等协议、GBN 协议和 SR 协议（发送窗口和接收窗口相等）传输数据，三个协议数据帧长相同，忽略确认帧长度，帧序号位数为 3 比特。若对应三个协议的发送方最大信道利用率分别是 U1、U2 和 U3，则 U1、U2 和 U3 满足的关系是（ ）', '[''U1 ≤ U2 ≤ U3'', ''U1 ≤ U3 ≤ U2'', ''U2 ≤ U3 ≤ U1'', ''U3 ≤ U2 ≤ U1'']', "['B']", 'B
停等协议的信道利用率公式为：
$$
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3993, '2023年408考研真题第36题', '简单', '2023年408真题', '已知 10BaseT 以太网的争用时间片为 51.2us。若网卡在发送某帧时发生了连续 4 次冲突，则基于二进制指数退避算法确定的再次尝试重发该帧前等待的最长时间是（ ）', '[''51.2us'', ''204.8us'', ''768us'', ''819.2us'']', "['C']", 'C
...
为$15 × 51.2us = 768us$。选择 C 选项。', 9, 'Mogullzr', '2026-01-29 18:56:25', 9, NULL, '2026-01-29 18:56:25', 1, 1);
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3994, '2023年408考研真题第37题', '简单', '2023年408真题', '若甲向乙发送数据时采用 CRC 校验，生成多项式为$G(x) = x^{4} + x + 1$（即 G=10011），则乙接收到下列比特串时，可以断定其在传输过程中未发生错误的是（ ）', '[''101110000'', ''101110100'', ''101111000'', ''101111100'']', "['D']", 'D
在循环冗余码中：若接收端将接收到的比特串除以 G(x)，余数为 0，则认为“未发生错误”。即，我们要对每一个选项做模 2 除法（异或除法），看余数是否为 0。经过计算，可以得到 D 选项余数为 0，所以答案选择 D。', 9, 'Mogullzr', '2026-01-29 18:56:25', 9, NULL, '2026-01-29 18:56:25', 1, 1);
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/f33410dc72cb4073bde2dfe7b8cbcdf7.svg)
', '[''192.168.0.33'', ''192.168.0.35'', ''192.168.0.1'', ''192.168.0.3'']', "['A']", 'A
...
192.163.0.33，所以本题正确答案为选项 A。', 9, 'Mogullzr', '2026-01-29 18:56:26', 9, NULL, '2026-01-29 18:56:26', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1304', '3995');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3996, '2023年408考研真题第39题', '简单', '2023年408真题', '主机 168.16.84.24/20 所在子网的最小可分配地址和最大可分配地址分别是（ ）', '[''168.16.80.1，168.16.84.254'', ''168.16.80.1，168.16.95.254'', ''168.16.84.1，168.16.84.254'', ''168.16.84.1，168.16.95.254'']', "['B']", 'B
...
如果超出了 TTL，则该 P 数据报要被丢弃。V 正确。综上所述，答案选择 D 选项。', 9, 'Mogullzr', '2026-01-29 18:56:26', 9, NULL, '2026-01-29 18:56:26', 1, 1);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1325', '3997');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3998, '2023年408考研真题第41题', '简单', '2023年408真题', '已知有向图 G 采用邻接矩阵存储，类型定义如下：typedefstruct{// 图的类型定义intnumVertices,numEdges;// 图中顶点数和有向边数charVerticesList[MAXV];// 顶点表，MAXV 为已定义常量intEdge[MAXV][MAXV];// 邻接矩阵}MGraph;将图中出度大于入度的顶点称为 K 顶点。例如在题 41 图中，顶点 a 和 b 都是 K 顶点。
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/4b830998db214165a53272dbd3a5be19.svg)
设计算法int printVertices(MGraph G)对给定任意非空有向图 G，输出 G 中所有 K 顶点的算法，并返回 K 顶点的个数。(1) 给出算法的设计思想。(2) 根据算法思想，写出 C/C++ 描述，并注释。', '[]', '', '1）采用邻接矩阵表示有向图时，一行中 1 的个数为该行对应顶点的出度，一列中 1 的个数为该列对应顶点的入度。使用一个初值为 0 的计数器记录 K 顶点的个数。对图 G 的每个顶点，根据邻接矩阵计算其出度 outdegree 和入度 indegree。若 outdegree - indegree > 0，则输出该顶点且计数器加 1。最后返回计数器的值。
2）算法实现
intprintVertices(MGraphG){// K 顶点的个数intcount=0;for(intv=0;v<numVertices;v++){// v 顶点的入度和出度intindegree=0;intoutdegree=0;// 统计出度for(inti=0;i<numVertices;i++){outdegree+=G.Edge[v][i];}// 统计入度for(inti=0;i<numVertices;i++){indegree+=G.Edge[i][v];}if(outdegree>indegree){printf("%s ",G.VerticesList[v]);count++;}}returncount;}
', 9, 'Mogullzr', '2026-01-29 18:56:28', 9, NULL, '2026-01-29 18:56:28', 1, 0);
INSERT INTO byteoj.problem_math408_tags (tag_id, problem_id)VALUES ('1193', '3998');
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (3999, '2023年408考研真题第42题', '简单', '2023年408真题', '对含有 n（n > 0）个记录的文件进行外部排序，采用置换 - 选择排序生成初始归并段时需要使用一个工作，工作区中能保存 m 个记录，请回答下列问题，(1) 如果文件中由 19 个记录，其关键字是 51, 94, 37, 92, 14, 63, 15, 99, 48, 56, 23, 60, 31, 17, 43, 8, 90, 166, 100；当 m=4 时，可以生成几个初始归并段，各是什么？(2) 对任意的 m 个（n > m > 0），生成的第一个初始归并段的长度最大值和最小值分别是多少？', '[]', '', '1）参考置换选择排序，该题的关键字序列可生成 3 个初始归并段，分别是：
...
INSERT INTO byteoj.problem_math408_bank (problem_id, problem_name, difficulty_name, source_name, description, options, correct_answer, analysis, create_by_id, create_by_name, create_time, update_by_id, update_by_name, update_time, status, option_type) VALUES (4001, '2023年408考研真题第44题', '简单', '2023年408真题', '上题中 C 程序段在计算机 M 上的部分机器级代码如下，每个机器级代码行中依次包含指令序号、虚拟地址、机器指令和汇编指令。请回答下列问题。(1) 第 20 条指令的虚拟地址是多少？(2) 已知第 2 条 jmp 和第 7 条 jge 都是跳转指令，其操作码分别是 EBH 和 7DH，跳转地址分别为 0040 1084H、0040 10BCH，这两条指令都采用什么寻址方式？给出第 2 条指令 jmp 的跳转目标地址计算过程。(3) 已知第 19 条 mov 指令的功能是“a[i][j]←10”，其中 ecx 和 edx 为寄存器名，0042 2000H 是数组 a 的首地址，指令中源操作数采用什么寻址方式？已知 edx 中存放的是变量 j，ecx 中存放的是什么？根据该指令的机器码判断计算机 M 采用的是大端还是小端方式。(4) 第一次执行第 19 条指令时，取指令过程中是否会发生缺页异常？为什么？', '[]', '', '1）第 20 条指令的虚拟地址为 0040 10B9H。
...
![图片](http://mogullzr001.oss-cn-beijing.aliyuncs.com/2026/01/6454892b26c445ebbf9f3c6e7f81e474.svg)
(1) FTP 的控制连接是持久的还是非持久的？FTP 的数据连接是持久的还是非持久的？H 登录服务器时，建立的 FTP 连接是数据连接还是控制连接。(2) H 通过数据连接发送 F 时，F 的第一个字节序号是多少？在断开数据连接的过程中，FTP 发送的第二次挥手的 ACK 序号是？(3) F 发送过程中，当 H 收到确认序号为 2101 的确认时，H 的拥塞调整为多少？收到确认序号为 7101 的确认段时，H 的拥窗口调整为多少(4) H 从请求建立数据连接开始，到确认 F 已被服务全部接收为止，至少要多长时间？期间应用层数平均发送速率是多少？', '[]', '', '1）FTP 的控制连接是持久的；数据连接是非持久的；H 登录 FTP 服务器时，建立的 TCP 连接是控制连接。
...
', 9, 'Mogullzr', '2026-01-29 18:56:31', 9, NULL, '2026-01-29 18:56:31', 1, 0);
