...
//                  }
    ...
    @Override
    public boolean courseUserJoin(Long courseId, Long uuid) {
        QueryWrapper<Course> courseQueryWrapper = new QueryWrapper<>();
        courseQueryWrapper.eq("course_id", courseId);
        ...
        if (courseUserAcProblem != null) {
            throw new BusinessException(ErrorCode.FORBIDDEN_ERROR, "请勿重复报名课程");
        } else {
            courseUserAcProblem = new CourseUserAcProblem();

            courseUserAcProblem.setCourse_id(courseId);
            courseUserAcProblem.setUuid(uuid);
            courseUserAcProblem.setNum(0L);

            return courseUserAcProblemMapper.insert(courseUserAcProblem) == 1;
        }
    }
}
