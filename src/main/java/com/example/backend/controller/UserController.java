...
//    }
    ...
    @PostMapping("/authorize/redirect")
    private BaseResponse<Boolean> userLoginByGithub(String code, HttpServletRequest httpServletRequest) {
        if (httpServletRequest == null){
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "信息不能为空");
        }

        boolean result = userService.UserLoginByGithub(code, Client_id, Client_secret, httpServletRequest);
        return ResultUtils.success(result);
    }
