...
public class CourseController {
    ...
    private BaseResponse<Boolean> courseUserJoin(Long course_id, HttpServletRequest httpServletRequest) {
        if (httpServletRequest == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "信息不能为空");
        }

        User user = userService.getLoginUser(httpServletRequest);
        boolean result = courseService.courseUserJoin(course_id, user.getUuid());
    ...
    private BaseResponse<Boolean> courseAdminUserSet(@RequestBody CourseUserRequest courseUserRequest,
