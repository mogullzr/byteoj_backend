...
public class LogController {
    ...
    @GetMapping("/user")
    private BaseResponse<LogUserInfoVo> logUserInfoVoGet(HttpServletRequest httpServletRequest) throws IOException {
        Boolean isAdmin = userService.isAdmin(httpServletRequest);
        if (!isAdmin) {
            throw new BusinessException(ErrorCode.NOT_AUTH_ERROR, "对不起，你没有操作权限");
        }
//        LogUserInfoVo logUserInfoVo = logService.logUserInfoVoGet();
